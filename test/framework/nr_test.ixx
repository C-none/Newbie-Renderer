...
[[noreturn]] inline void throwFailure(std::string_view message,
...
inline auto runOne(const TestCase &testCase) -> TestResult
{
    ...
    catch (const std::exception &exception)
    {
        assertionCounterPtr() = nullptr;
        return TestResult{
            .name = testCase.name, .passed = false, .assertions = localAssertions, .detail = exception.what()};
    }
    catch (...)
    {
        assertionCounterPtr() = nullptr;
        return TestResult{
            .name = testCase.name, .passed = false, .assertions = localAssertions, .detail = "unknown exception"};
    }
}
...
inline void require(bool condition, std::string_view message = "require failed",
                    const std::source_location &location = std::source_location::current())
{
...
inline void requireEqual(const L &lhs, const R &rhs, std::string_view message = "expected lhs == rhs",
                         const std::source_location &location = std::source_location::current())
{
...
inline int runAll(std::ostream &output = std::cout)
{
    ...
    const auto passedCount =
        std::ranges::count_if(results, [](const detail::TestResult &result) { return result.passed; });
    const auto failedCount = static_cast<int>(results.size()) - static_cast<int>(passedCount);
    const auto assertionCount =
        std::transform_reduce(results.begin(), results.end(), 0, std::plus{},
                              [](const detail::TestResult &result) { return result.assertions; });

    output << "[nr_test] passed=" << passedCount << ", failed=" << failedCount << ", assertions=" << assertionCount
           << '\n';
