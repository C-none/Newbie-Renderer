...
[[nodiscard]] lua::SandboxConfig sandboxConfig(HostTrace &trace)
{
    ...
    config.hostFunctions = {
        ...
        lua::HostFunctionBinding{
            ...
            .function =
                [](std::span<const lua::Value> arguments) {
                    return arguments.empty() ? lua::HostCallResult::suspend()
                                             : lua::HostCallResult::failure("next expects no arguments");
                },
        },
        lua::HostFunctionBinding{
...
const nr::test::CaseRegistrar exactAllowlistCase{
    "Lua sandbox exposes only the reviewed per-function allowlist", [] {
...
const nr::test::CaseRegistrar readOnlyProxyCase{
    "Lua namespace proxies reject direct and library-assisted mutation", [] {
        constexpr auto attempts = std::array{
            "nr.options.snapshot = 7",
            "nr.options.extra = 7",
            "string.byte = nil",
            "table.insert(nr.options, \"escape\")",
            "table.move({7}, 1, 1, 1, nr.options)",
            "_ENV.escape = true",
        };
        std::ranges::for_each(attempts, [](std::string_view source) {
            ...
            nr::test::requireEqual(resumed.status, lua::ResumeStatus::failed,
                                   "proxy mutation must terminate the script");
            nr::test::require(resumed.detail.contains("read-only"), resumed.detail);
        });
    }};

const nr::test::CaseRegistrar coroutineCase{
    "Lua frame yields resume one host-owned coroutine in order", [] {
        auto trace = HostTrace{};
        auto sandbox = lua::Sandbox{};
...
const nr::test::CaseRegistrar loaderAndQuotaCase{
    "Lua sandbox enforces text source memory instruction wall and conversion bounds", [] {
        {
            auto trace = HostTrace{};
            auto sandbox = lua::Sandbox{};
        ...
        }
        {
            ...
            auto started =
                sandbox.start("local value = string.rep(\"x\", 8 * 1024 * 1024)", "@memory.lua", std::move(config));
            nr::test::require(started.started, started.detail);
        ...
        }
        {
            auto trace = HostTrace{};
            auto sandbox = lua::Sandbox{};
            auto started = sandbox.start(R"lua(nr.options.apply(string.rep("x", 65537), true, {}))lua",
                                         "@string-boundary.lua", sandboxConfig(trace));
            nr::test::require(started.started, started.detail);
            auto resumed = sandbox.resume();
            nr::test::requireEqual(resumed.status, lua::ResumeStatus::failed);
...
const nr::test::CaseRegistrar offlineHostCase{
    "Offline Lua uses automation-root paths and the shared option admission slot", [] {
        ...
        nr::test::require(
            !automation::resolveAutomationScriptPath("../test/unit/automation/outside_automation.lua").has_value());
        nr::test::require(
            !automation::resolveAutomationScriptPath(automation::automationRootPath() / "test/offline_contract.lua")
                 .has_value());
...
const nr::test::CaseRegistrar finalApplyAndLogQuotaCase{
    "Offline Lua completion does not drain a final apply and log quotas fail the script", [] {
        {
            auto system = offlineSystem();
            auto host = automation::OfflineLuaHost{};
