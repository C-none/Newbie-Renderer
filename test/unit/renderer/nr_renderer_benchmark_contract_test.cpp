...
void appendValidFrame(AuditInput &input)
{
    ...
    input.passes.emplace_back(nr::renderer::RendererBenchmarkGpuPass{.frameOrdinal = 2u, .passIndex = 0u, .debugName = "main", .milliseconds = 1.0});
    input.statuses.emplace_back(nr::renderer::RendererBenchmarkGpuFrameStatus{.frameOrdinal = 2u, .expectedPassCount = 1u, .availablePassCount = 1u, .complete = true});
    input.accelerationStructures.emplace_back(nr::renderer::RendererBenchmarkAsTelemetry{.recorded = true, .available = true});
}

const nr::test::CaseRegistrar quantileCase{"renderer benchmark type 7 quantiles handle empty and singleton samples", [] {
...
const nr::test::CaseRegistrar distributionCase{"renderer benchmark distribution uses population standard deviation", [] {
...
const nr::test::CaseRegistrar executeTelemetryCase{"renderer benchmark validates execute telemetry accounting and counts", [] {
                                                      ...
                                                      sample.executeAccountedMainThreadMilliseconds = nr::renderer::rendererBenchmarkExecuteAccountedMainThreadMilliseconds(sample.execute);
                                                      sample.executeUnclassifiedMilliseconds = sample.cpu.executeMilliseconds - sample.executeAccountedMainThreadMilliseconds;
...
const nr::test::CaseRegistrar executeSchemaCase{"renderer benchmark execute CSV and summary schemas are complete", [] {
...
const nr::test::CaseRegistrar gpuStatusAuditCase{"renderer benchmark quality audit classifies GPU status failures", [] {
                                                     ...
                                                     extra.statuses.emplace_back(nr::renderer::RendererBenchmarkGpuFrameStatus{.frameOrdinal = 2u, .expectedPassCount = 1u, .availablePassCount = 1u, .complete = true});
...
const nr::test::CaseRegistrar gpuPassAuditCase{"renderer benchmark quality audit classifies GPU pass failures", [] {
                                                   auto duplicate = validAuditInput();
                                                   ...
                                                   extraFrame.passes.emplace_back(nr::renderer::RendererBenchmarkGpuPass{.frameOrdinal = 2u, .passIndex = 0u, .debugName = "main", .milliseconds = 1.0});
...
const nr::test::CaseRegistrar gpuSchemaAuditCase{"renderer benchmark quality audit detects every GPU schema field drift", [] {
...
const nr::test::CaseRegistrar frameAuditCase{"renderer benchmark quality audit rejects frame contract violations", [] {
                                                 ...
                                                 ordinal.frames.back().frameOrdinal = ordinal.frames.front().frameOrdinal;
                                                 ...
                                                 nan.frames.front().cpu.totalMilliseconds = std::numeric_limits<double>::quiet_NaN();
                                                 nr::test::require(!audit(nan).framesValid);
...
const nr::test::CaseRegistrar nodeAndAsAuditCase{"renderer benchmark quality audit validates node and AS telemetry", [] {
