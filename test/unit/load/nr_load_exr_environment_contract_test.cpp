  ...
  public:
    explicit TemporaryExr(std::string_view stem)
    {
        ...
        path_ = std::filesystem::temp_directory_path() /
                std::format("nr_{}_{}.exr", stem, stamp);
    }
...
[[nodiscard]] bool writeFloatExr(
    const std::filesystem::path& path,
    std::span<FloatRgb> pixels,
    std::uint32_t width,
    std::uint32_t height,
    bool includeBlue)
{
    try
    {
        ...
        header.channels().insert(
            "R",
            nr::dependency::openexr::Channel(nr::dependency::openexr::floatPixelType));
        header.channels().insert(
            "G",
            nr::dependency::openexr::Channel(nr::dependency::openexr::floatPixelType));
        if (includeBlue)
        ...
        frameBuffer.insert(
            ...
            nr::dependency::openexr::Slice(
                ...
                reinterpret_cast<char*>(&pixels.front().r),
                xStride,
                yStride));
        frameBuffer.insert(
            "G",
            nr::dependency::openexr::Slice(
                nr::dependency::openexr::floatPixelType,
                reinterpret_cast<char*>(&pixels.front().g),
                xStride,
                yStride));
        if (includeBlue)
        {
            frameBuffer.insert(
                ...
                nr::dependency::openexr::Slice(
                    nr::dependency::openexr::floatPixelType,
                    reinterpret_cast<char*>(&pixels.front().b),
                    xStride,
                    yStride));
        }
    ...
    catch (const std::exception& error)
    {
        nr::nrInfo<nr::LogLevel::error>(std::format(
            "Failed to write OpenEXR test fixture '{}': {}",
            path.generic_string(),
            error.what()));
        return false;
    }
    ...
    {
        nr::nrInfo<nr::LogLevel::error>(std::format(
            "Failed to write OpenEXR test fixture '{}': unknown OpenEXR error.",
            path.generic_string()));
        return false;
    }
}

[[nodiscard]] float encodedHalf(
    const nr::resource::EnvironmentMap& environment,
    ...
    std::size_t channelIndex)
{
    ...
    auto const offset =
        (pixelIndex * 4u + channelIndex) * sizeof(nr::dependency::imath::Half);
...
const nr::test::CaseRegistrar scaledFloatExrCase{
    ...
    [] {
        auto source = std::array{
            FloatRgb{.r = -1.0e-8f, .g = 1.0f, .b = 2.0f},
            FloatRgb{.r = 3.0f, .g = 4.0f, .b = 5.0f},
            FloatRgb{.r = 75'000.0f, .g = 100.0f, .b = 200.0f},
            FloatRgb{.r = 7.0f, .g = 8.0f, .b = 9.0f},
            FloatRgb{.r = 10.0f, .g = 11.0f, .b = 12.0f},
            FloatRgb{.r = 13.0f, .g = 14.0f, .b = 15.0f},
            FloatRgb{.r = 16.0f, .g = 17.0f, .b = 18.0f},
            FloatRgb{.r = 19.0f, .g = 20.0f, .b = 21.0f},
        };
        auto file = TemporaryExr{"scaled_environment"};
        nr::test::require(
            writeFloatExr(file.path(), source, 4u, 2u, true),
        ...
        nr::test::require(
            environment.radiance.format == vk::Format::eR16G16B16A16Sfloat,
        ...
        nr::test::requireEqual(
            environment.radiance.levels.front().bytes.size(),
            std::size_t{4u * 2u * 4u * sizeof(nr::dependency::imath::Half)});
        nr::test::require(
            near(environment.radianceDecodeScale, 1.25f, 1.0e-6f),
        ...
        nr::test::require(
            near(
                encodedHalf(environment, 2u, 0u) * environment.radianceDecodeScale,
                75'000.0f,
                64.0f),
            "scaled HALF peak should reconstruct source radiance");
        nr::test::require(
            near(
                encodedHalf(environment, 1u, 2u) * environment.radianceDecodeScale,
                5.0f,
                0.01f),
...
const nr::test::CaseRegistrar missingChannelCase{
    ...
    [] {
        ...
        nr::test::require(
            writeFloatExr(file.path(), source, 1u, 1u, false),
            "RG OpenEXR fixture should be written");
        ...
        nr::test::require(
            result.error().code == nr::load::LoadErrorCode::textureDataUnsupported,
            "missing B should report textureDataUnsupported");
        nr::test::require(
            result.error().message.find("'B'") != std::string::npos,
