...
[[nodiscard]] std::shared_ptr<const OptionCatalog> catalog(std::span<const OptionDefinition> definitions)
{
    ...
    std::ranges::for_each(definitions, [&](auto const &definition) { nr::test::require(builder.add(definition), "definition should be accepted"); });
...
[[nodiscard]] OptionAvailabilityMap allAvailable(const OptionCatalog &value)
{
    ...
    std::ranges::for_each(value.definitions(), [&](auto const &entry) { result.emplace(entry.first, OptionAvailability{.available = true, .reason = {}}); });
    return result;
}
...
[[nodiscard]] std::unique_ptr<OptionSystem> initializedSystem(AuthorityMode mode = AuthorityMode::human)
{
    ...
    nr::test::require(system->initializeSession(session, allAvailable(*session)).committed, "session catalog should initialize");
    return system;
}

template <WireValueAlternative T> [[nodiscard]] OptionMutationRequest request(OptionSystem &system, OptionKey<T> key, T value, MutationOrigin origin = MutationOrigin::imgui)
{
...
[[nodiscard]] std::shared_ptr<const OptionFrameSnapshot> finishFrame(OptionSystem &system, std::optional<FrameEffect> effect = {})
{
    auto currentCatalog = system.activeCatalog();
    return system.publishRenderableFrame(allAvailable(*currentCatalog), std::move(effect));
}

const nr::test::CaseRegistrar schemaAndCatalogCase{"option catalog validates IDs defaults schemas and snapshot bounds", [] {
                                                       ...
                                                       nr::test::require(!clipDefinition->schema.validate(invalidClip).valid, "clip cross-field validation should reject far <= near");
                                                       ...
                                                       nr::test::require(!clipDefinition->schema.validate(tooLargeClip).valid, "camera clip values must remain representable by the float renderer camera");

                                                       auto poseDefinition = std::ranges::find_if(sessionDefinitions, [](auto const &definition) { return definition.id == optionId(keys::viewerCameraPose); });
                                                       ...
                                                       nr::test::require(maximumValueBuilder.add(makeDefinition(keys::viewerModelSource, OptionSchema::string(64u * 1024u), std::string{}, OptionScope::session)));
                                                       nr::test::require(!maximumValueBuilder.build(256u * 1024u).valid(), "snapshot preflight must bound the schema's largest value, not only its default");
                                                   }};

const nr::test::CaseRegistrar fixedCatalogCase{"fixed option catalog has one canonical DLSS quality and explicit frame effects", [] {
                                                   auto session = sessionCatalog();
...
const nr::test::CaseRegistrar singleSlotCase{"option admission has one slot and one irreversible attempt per frame", [] {
                                                 auto system = initializedSystem();
...
const nr::test::CaseRegistrar proofCase{"option admission rejects missing stale and mismatched binding proofs", [] {
                                            auto system = initializedSystem();
                                            ...
                                            nr::test::requireEqual(system->trySchedule(std::move(bothMismatch)).reason, ScheduleRejectReason::bindingProofMismatch);

                                            auto bothMatch = request(*system, keys::viewerWindowFullscreen, true);
                                            bothMatch.binding.snapshotToken = binding.snapshotToken;
                                            nr::test::require(system->trySchedule(std::move(bothMatch)).started, "matching epoch and token proofs must be accepted together");
                                            auto frame = system->beginRenderableFrame();
                                            nr::test::require(frame && frame->mutation);
...
const nr::test::CaseRegistrar snapshotTokenIdentityCase{"snapshot tokens are session-bound stable binding proofs", [] {
                                                            ...
                                                            nr::test::require(firstToken != secondToken, "two OptionSystem sessions in one process must never share a snapshot token");
                                                            ...
                                                            nr::test::requireEqual(first->trySchedule(std::move(mixedProof)).reason, ScheduleRejectReason::bindingProofMismatch);

                                                            auto frame = first->beginRenderableFrame();
                                                            nr::test::require(frame && !frame->mutation);
                                                            auto nextSnapshot = finishFrame(*first);
                                                            nr::test::requireEqual(nextSnapshot->snapshotToken, firstToken, "ordinary frame publication must not invalidate a binding proof");
                                                            ...
                                                            nr::test::require(replacementSnapshot->snapshotToken != firstToken, "graph replacement must invalidate the previous session binding token");
                                                        }};

const nr::test::CaseRegistrar concurrentSingleSlotCase{"concurrent admission linearizes to one started mutation", [] {
                                                           auto system = initializedSystem();
                                                           ...
                                                           nr::test::requireEqual(std::ranges::count(results, true, &ScheduleResult::started), std::ptrdiff_t{1}, "exactly one racing request may reserve the pending slot");
...
const nr::test::CaseRegistrar executionRevalidationCase{"moved frame mutation is rejected after a binding generation changes", [] {
                                                            auto system = initializedSystem();
                                                            ...
                                                            nr::test::requireEqual(system->validateForExecution(*frame->mutation), ScheduleRejectReason::staleBinding, "execution must revalidate the binding under the same state mutex");
                                                            auto commit = system->commitCanonical(std::move(*frame->mutation));
                                                            nr::test::require(!commit.committed);
...
const nr::test::CaseRegistrar catalogAdmissionRaceCase{"catalog replacement and admission race has no lost or misbound mutation", [] {
                                                           ...
                                                           std::ranges::for_each(std::views::iota(0u, 32u), [&](auto) {
                                                               ...
                                                               if (schedule.started)
                                                               {
                                                                   nr::test::require(frame->mutation.has_value(), "a request linearized before gate close must be moved into that frame");
                                                                   nr::test::requireEqual(frame->mutation->sequence(), schedule.sequence);
                                                                   nr::test::requireEqual(system->validateForExecution(*frame->mutation), ScheduleRejectReason::staleBinding, "catalog replacement must invalidate the already-moved old binding");
                                                                   nr::test::require(system->discardMutation(std::move(*frame->mutation)));
                                                               }
                                                               else
                                                               {
...
const nr::test::CaseRegistrar crossOptionCase{"cross-option validation rejects DLSS bypass combinations without implicit correction", [] {
                                                  auto system = initializedSystem();
                                                  ...
                                                  nr::test::requireEqual(*system->snapshot()->find(keys::dlssQuality), std::string{"quality"}, "invalid bypass must not rewrite quality");
...
const nr::test::CaseRegistrar minimizedCase{"skipping option frame preserves pending mutation and immutable snapshot", [] {
                                                auto system = initializedSystem();
                                                ...
                                                nr::test::require(!*after->find(keys::viewerWindowFullscreen), "discarded attempt must not change canonical value");

                                                auto nextFrame = system->beginRenderableFrame();
                                                nr::test::require(nextFrame && !nextFrame->mutation, "failed/discarded mutation must never be restored or retried");
                                                (void)finishFrame(*system);
                                            }};

const nr::test::CaseRegistrar availabilityCase{"published conservative availability gates only mutation admission", [] {
                                                   auto system = initializedSystem();
                                                   auto frame = system->beginRenderableFrame();
                                                   nr::test::require(frame && !frame->mutation);
                                                   ...
                                                   nr::test::requireEqual(unavailableSnapshot->findAvailability(optionId(keys::viewerWindowFullscreen))->reason, std::string{"window_transition_busy"});
                                                   nr::test::requireEqual(system->trySchedule(request(*system, keys::viewerWindowFullscreen, true)).reason, ScheduleRejectReason::unavailable);
                                                   nr::test::require(unavailableSnapshot->find(keys::viewerWindowFullscreen) != nullptr, "unavailability must not hide the option from read-only snapshots");
                                                   ...
                                                   nr::test::requireEqual(boundedSnapshot->findAvailability(optionId(keys::viewerWindowFullscreen))->reason, std::string{"invalid_availability_reason"}, "snapshot availability reasons must remain inside the preflighted response bound");
                                               }};

const nr::test::CaseRegistrar graphReplacementCase{"graph replacement preserves session values resets graph defaults and invalidates old proof", [] {
                                                       auto system = initializedSystem();
                                                       ...
                                                       nr::test::require(oldSnapshot->find(keys::pathTracingMaxSurfaceBounces) == nullptr, "held old snapshot must remain immutable");
                                                       ...
                                                       nr::test::require(system->trySchedule(request(*system, keys::pathTracingMaxSurfaceBounces, std::uint64_t{24u})).started);
                                                       frame = system->beginRenderableFrame();
                                                       nr::test::require(system->commitCanonical(std::move(*frame->mutation)).committed);
                                                       ...
                                                       nr::test::requireEqual(*reset->find(keys::viewerWindowFullscreen), false, "session value must remain present across graph replacement");
                                                   }};

const nr::test::CaseRegistrar graphMutationReplacementCase{"graph-switch mutation commits its session value and catalog under the old binding", [] {
                                                               auto system = initializedSystem();
                                                               auto graphDefinitions = makePathTracingDefinitions();
                                                               auto graph = catalog(graphDefinitions);
                                                               auto frame = system->beginRenderableFrame();
                                                               ...
                                                               nr::test::require(system->trySchedule(request(*system, keys::viewerPipelineSelected, std::string{"rtobject"})).started);
                                                               frame = system->beginRenderableFrame();
...
const nr::test::CaseRegistrar effectCase{"frame effects are one-shot and never become a retained operation result", [] {
                                             auto system = initializedSystem();
                                             ...
                                             nr::test::require(system->trySchedule(request(*system, keys::presentCaptureExr, OptionWireValue::Object{})).started);
                                             frame = system->beginRenderableFrame();
                                             ...
                                             nr::test::requireEqual(withEffect->findAvailability(optionId(keys::presentCaptureExr))->available, false, "effect must publish provisional busy availability");

                                             frame = system->beginRenderableFrame();
                                             nr::test::require(frame && !frame->mutation);
                                             auto next = finishFrame(*system);
                                             nr::test::require(!next->effect.has_value(), "effect must disappear from the next snapshot");
                                             nr::test::require(next->findAvailability(optionId(keys::presentCaptureExr))->available, "capture availability may recover only in a later published snapshot");
                                         }};

const nr::test::CaseRegistrar derivedModelCase{"model commit atomically applies the explicit camera reset exception", [] {
                                                   auto system = initializedSystem();
...
const nr::test::CaseRegistrar shutdownCase{"shutdown abandons one pending mutation and keeps the last snapshot readable", [] {
                                               auto system = initializedSystem();
                                               ...
                                               nr::test::requireEqual(system->trySchedule(request(*system, keys::viewerWindowFullscreen, true)).reason, ScheduleRejectReason::shutdown);
                                               nr::test::require(!system->beginRenderableFrame().has_value());
                                           }};

const nr::test::CaseRegistrar machineRecordCase{"option machine records are compact UTF-8 JSON with stable field names", [] {
                                                    ...
                                                    nr::test::requireEqual(jsonStringField(recordJson, "option_id"), std::string_view{"render.present.capture_exr"});
                                                    ...
                                                    serialized = serializeMachineRecord(record);
                                                    recordJson = parseJsonValue(serialized);
                                                    nr::test::requireEqual(jsonStringField(recordJson, "request_id"), std::string_view{"request_id_exceeded_limit"});
                                                    nr::test::requireEqual(jsonStringField(recordJson, "reason"), std::string_view{"reason_exceeded_limit"});
                                                }};

const nr::test::CaseRegistrar authorityCase{"authority mode keeps all entry points on one admission API", [] {
                                                auto agent = initializedSystem(AuthorityMode::agent);
                                                nr::test::requireEqual(agent->trySchedule(request(*agent, keys::viewerWindowFullscreen, true)).reason, ScheduleRejectReason::unauthorizedOrigin);
...
const nr::test::CaseRegistrar atomicReadCase{"published option snapshots support concurrent immutable readers", [] {
                                                 auto system = initializedSystem();
