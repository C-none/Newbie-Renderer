...
[[nodiscard]] options::OptionAvailabilityMap allAvailable(const options::OptionCatalog &catalog)
{
    ...
    std::ranges::for_each(catalog.definitions(), [&](auto const &entry) { result.emplace(entry.first, options::OptionAvailability{.available = true, .reason = {}}); });
    return result;
}
...
const nr::test::CaseRegistrar jsonBoundaryCase{"dependency JSON stays strict bounded and round-trippable", [] {
                                                   ...
                                                   nr::test::requireEqual(json::serializeJson(json::JsonValue{std::move(invalidUtf8)}, invalidOutput, 256u), json::JsonError::invalidUtf8);
                                                   nr::test::require(invalidOutput.empty());
                                               }};

const nr::test::CaseRegistrar handshakeCase{"WebSocket handshake policy enforces loopback endpoint auth Origin and one controller", [] {
                                                ...
                                                nr::test::requireEqual(network::evaluateHandshake(origin, token), network::HandshakeRejectReason::originForbidden);

                                                auto wrongPath = valid;
                                                wrongPath.target = "/v1/options/";
                                                nr::test::requireEqual(network::evaluateHandshake(wrongPath, token), network::HandshakeRejectReason::wrongTarget);
                                                ...
                                                nr::test::requireEqual(network::evaluateHandshake(wrongToken, token), network::HandshakeRejectReason::unauthorized);
                                                ...
                                                nr::test::requireEqual(network::evaluateHandshake(headers, token), network::HandshakeRejectReason::headersTooLarge);
                                            }};

const nr::test::CaseRegistrar payloadCase{"WebSocket payload policy uses RFC close codes after message reassembly", [] {
                                              nr::test::require(network::evaluatePayload(network::PayloadKind::text, true, 256u * 1024u).accepted);
                                              nr::test::requireEqual(*network::evaluatePayload(network::PayloadKind::binary, true, 1u).closeCode, network::WebSocketCloseCode::unsupportedData);
                                              nr::test::requireEqual(*network::evaluatePayload(network::PayloadKind::text, false, 1u).closeCode, network::WebSocketCloseCode::invalidPayload);
                                              nr::test::requireEqual(*network::evaluatePayload(network::PayloadKind::text, true, 256u * 1024u + 1u).closeCode, network::WebSocketCloseCode::messageTooBig);
                                          }};

const nr::test::CaseRegistrar readMethodsCase{"WebSocket protocol exposes only snapshot-backed read methods", [] {
                                                  auto system = agentSystem();
                                                  auto protocol = interaction::OptionRpcProtocol{*system};
                                                  auto slot = std::string{};

                                                  auto describe = protocol.handleText(R"({"jsonrpc":"2.0","id":"describe","method":"session.describe","params":{}})", {}, slot);
                                                  ...
                                                  nr::test::requireEqual(std::get<std::string>(jsonField(describeResult, "authority_mode").storage), std::string{"agent"});
                                                  nr::test::require(!std::get<bool>(jsonField(describeResult, "tasks").storage));
                                                  nr::test::requireEqual(
                                                      std::get<std::string>(jsonField(describeResult, "final_result_channel").storage),
                                                      std::string{"rotating-ndjson-file"});
                                                  auto const finalResultPath =
                                                      std::get<std::string>(jsonField(describeResult, "final_result_path").storage);
                                                  nr::test::require(
                                                      finalResultPath.ends_with("build/app/logs/options.ndjson"),
                                                      "final result path should identify the active option NDJSON log");
                                                  nr::test::requireEqual(
                                                      std::get<std::string>(jsonField(describeResult, "final_result_schema").storage),
                                                      std::string{"NR_OPTION_V1"});

                                                  auto snapshot = protocol.handleText(R"({"jsonrpc":"2.0","id":"snapshot","method":"option.snapshot","params":{}})", {}, slot);
                                                  ...
                                                  nr::test::requireEqual(std::get<std::string>(jsonField(record, "id").storage), std::string{"viewer.window.fullscreen"});
                                              }};

const nr::test::CaseRegistrar admissionCase{"started response is prepared before admission and transport loss never rolls back", [] {
                                                auto system = agentSystem();
                                                auto protocol = interaction::OptionRpcProtocol{*system};
                                                auto slot = std::string{};
                                                ...
                                                nr::test::requireEqual(std::get<std::string>(jsonField(startedResult, "status").storage), std::string{"started"});
                                                nr::test::require(system->hasPendingMutation());

                                                slot.clear();
                                                auto reconnectedProtocol = interaction::OptionRpcProtocol{*system};
                                                auto duplicate = reconnectedProtocol.handleText(applyRequest(system->liveBinding().bindingEpoch, "apply-2"), {}, slot);
...
const nr::test::CaseRegistrar fragmentedAndProfileCase{"reassembled fragmented requests work while batch notification and extra fields never schedule", [] {
                                                           ...
                                                           auto batch = protocol.handleText(R"([{"jsonrpc":"2.0","id":"batch","method":"option.snapshot","params":{}}])", {}, slot);
                                                           nr::test::require(batch.responseReady && !batch.mutationStarted);
                                                           nr::test::require(!system->hasPendingMutation());

                                                           auto notification = protocol.handleText(R"({"jsonrpc":"2.0","method":"option.apply","params":{"id":"viewer.window.fullscreen","value":true,"binding_epoch":1}})", {}, slot);
...
const nr::test::CaseRegistrar rateLimitCase{"transport rate rejection is explicit and cannot reserve the option slot", [] {
                                                auto system = agentSystem();
                                                auto protocol = interaction::OptionRpcProtocol{*system};
                                                auto slot = std::string{};
                                                auto result = protocol.handleText(applyRequest(system->liveBinding().bindingEpoch), network::MessageContext{.rateLimited = true}, slot);
                                                nr::test::require(result.responseReady && !result.mutationStarted);
                                                nr::test::require(!system->hasPendingMutation());
...
const nr::test::CaseRegistrar liveTransportHandshakeAndLockstepCase{"live WebSocket transport enforces handshake ownership lockstep fragmentation and reconnect", [] {
                                                                        auto const token = std::string{"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"};
                                                                        auto system = agentSystem();
                                                                        auto protocol = interaction::OptionRpcProtocol{*system};
                                                                        auto server = network::LoopbackWebSocketServer{};
                                                                        auto started = server.start(network::WebSocketServerConfig{.bearerToken = token},
                                                                                                    [&](std::string_view payload, const network::MessageContext &context, std::string &responseSlot) { return protocol.handleText(payload, context, responseSlot); });
                                                                        ...
                                                                        auto firstRequest = std::string{R"({"jsonrpc":"2.0","id":"live-describe","method":"session.describe","params":{}})"};
                                                                        ...
                                                                        nr::test::requireEqual(std::get<std::string>(jsonField(jsonObject(firstResponse), "id").storage), std::string{"live-describe"});

                                                                        auto fragmentedRequest = std::string{R"({"jsonrpc":"2.0","id":"live-snapshot","method":"option.snapshot","params":{}})"};
                                                                        ...
                                                                        nr::test::requireEqual(std::get<std::string>(jsonField(jsonObject(fragmentedResponse), "id").storage), std::string{"live-snapshot"});
...
const nr::test::CaseRegistrar liveTransportLostStartedResponseCase{"live transport loss after admission preserves the pending mutation and reports the lost started response once", [] {
                                                                       auto const token = std::string{"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"};
                                                                       auto system = agentSystem();
                                                                       auto protocol = interaction::OptionRpcProtocol{*system};
                                                                       ...
                                                                       auto started = server.start(
                                                                           std::move(config),
                                                                           [&, maximumResponseBytes](std::string_view payload, const network::MessageContext &context, std::string &responseSlot) {
                                                                       ...
                                                                       nr::test::require(system->hasPendingMutation(), "the apply request should reach admission before the transport is dropped");
                                                                       controller.abort();

                                                                       {
                                                                           auto lock = std::unique_lock{eventsMutex};
                                                                           auto const completed = eventsChanged.wait_for(lock, std::chrono::seconds{3}, [&] {
                                                                               auto const lostResponse = std::ranges::any_of(events, [](auto const &event) { return event.kind == network::TransportEventKind::responseWriteFailed && event.startedResponseLost; });
                                                                               auto const connectionClosed = std::ranges::any_of(events, [](auto const &event) { return event.kind == network::TransportEventKind::connectionClosed; });
                                                                           ...
                                                                           auto const lostEvents = std::ranges::count_if(events, [](auto const &event) { return event.startedResponseLost; });
                                                                           auto const duplicateClosedEvents = std::ranges::count_if(events, [](auto const &event) { return event.kind == network::TransportEventKind::connectionClosed && event.startedResponseLost; });
                                                                           nr::test::requireEqual(lostEvents, std::ptrdiff_t{1}, "the lost started response must be reported exactly once");
                                                                           nr::test::requireEqual(duplicateClosedEvents, std::ptrdiff_t{0}, "connection close must not repeat a consumed lost-response event");
                                                                       }
                                                                       ...
                                                                       nr::test::requireEqual(std::get<std::string>(jsonField(retryData, "reason").storage), std::string{"operation_busy"});
                                                                       nr::test::require(system->hasPendingMutation(), "the original admitted mutation should remain pending after reconnect");
                                                                       reconnected.close();
                                                                       server.stop();
                                                                   }};

const nr::test::CaseRegistrar liveTransportCloseCodeCase{"live WebSocket transport returns RFC close codes for binary invalid UTF-8 and oversized text", [] {
                                                             auto const token = std::string{"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"};
                                                             auto server = network::LoopbackWebSocketServer{};
                                                             auto started = server.start(network::WebSocketServerConfig{.bearerToken = token}, [](std::string_view, const network::MessageContext &, std::string &) { return network::TextMessageResult{}; });
                                                             nr::test::require(started.started, started.detail);

                                                             {
                                                                 ...
                                                                 nr::test::requireEqual(*closed.closeCode, static_cast<std::uint16_t>(network::WebSocketCloseCode::unsupportedData));
                                                             }

                                                             {
                                                                 ...
                                                                 nr::test::require(closed.closeCode.has_value(), closed.detail);
                                                                 nr::test::requireEqual(*closed.closeCode, static_cast<std::uint16_t>(network::WebSocketCloseCode::invalidPayload));
                                                             }

                                                             {
                                                                 ...
                                                                 nr::test::require(closed.closeCode.has_value(), closed.detail);
                                                                 nr::test::requireEqual(*closed.closeCode, static_cast<std::uint16_t>(network::WebSocketCloseCode::messageTooBig));
                                                             }
                                                             server.stop();
                                                         }};
} // namespace
