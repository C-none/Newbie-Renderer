...
const nr::test::CaseRegistrar sbtRepeatedGroupRecordsCase{
    "rhi rt SBT build plan owns only value layout metadata", [] {
        auto plan = nr::rhi::ShaderBindingTableBuildPlan{};
        {
            ...
            plan = nr::rhi::makeShaderBindingTableBuildPlan(desc);
        }

        nr::test::requireEqual(plan.hit.recordCount, 3u);
        nr::test::requireEqual(plan.hit.stride, 32u);
