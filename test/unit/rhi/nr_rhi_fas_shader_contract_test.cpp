...
void executePayloadPackingContract(const nr::rhi::SlangProgram &program)
{
    ...
    auto pipeline = device.pipeline().createComputePipeline(program);
    nr::test::require(pipeline.pipeline.valid() && pipeline.layout.valid() && pipeline.descriptorLayout.valid(), "payload packing contract should create a valid compute pipeline");
    ...
    auto resultBuffer = device.resourceFactory.createBuffer(bufferInfo, nr::rhi::MemoryUsage::GpuOnly, "payload_packing_contract_results");
    ...
    nr::test::require(rootCursor["gContractResults"].setObject(resultBuffer), "payload packing contract should bind its reflected result buffer");
    ...
    nr::rhi::bindPreparedResourcesToCommandBuffer(commandBuffer, vk::PipelineBindPoint::eCompute, pipeline.layout, bindingSets);
    ...
    auto readbackTicket = device.uploadReadback().readbackBuffer(resultBuffer, 0, resultByteSize, nr::rhi::QueueRole::Compute,
    ...
    std::ranges::for_each(std::views::iota(std::size_t{0u}, results.size()), [&](std::size_t resultIndex) { nr::test::require(results[resultIndex] == 1u, std::format("GPU payload packing contract failed: {} (value={})", payloadPackingResultNames[resultIndex], results[resultIndex])); });

    device.waitIdle();
}

const nr::test::CaseRegistrar materialFilterContractCase{"material FAS contract compiles both root policies with stable layout ABI", [] {
                                                             ...
                                                             nr::test::require(offProgram.valid() && onProgram.valid() && payloadPackingProgram.valid(), "material FAS and payload packing contracts should compile as separate single-entry programs");
                                                             nr::test::require(offProgram.entryPoint()->spirv != nullptr && !offProgram.entryPoint()->spirv->empty() && onProgram.entryPoint()->spirv != nullptr && !onProgram.entryPoint()->spirv->empty() &&
                                                                                   payloadPackingProgram.entryPoint()->spirv != nullptr && !payloadPackingProgram.entryPoint()->spirv->empty(),
                                                             ...
                                                             nr::test::require(rayPayloadLayout != nullptr, "material FAS contract should reflect ResolvedMaterialRayPayload");
                                                             ...
                                                             nr::test::require(rayPayloadSize == 128u && rayPayloadStride == 128u, std::format("packed ResolvedMaterialRayPayload should remain 32 uint32 lanes: size={}, stride={}", rayPayloadSize, rayPayloadStride));
                                                             auto *rayPayloadWrapperType = programLayout->findTypeByName("RayPayload");
                                                             auto *rayPayloadWrapperLayout = rayPayloadWrapperType != nullptr ? programLayout->getTypeLayout(rayPayloadWrapperType) : nullptr;
                                                             nr::test::require(rayPayloadWrapperLayout != nullptr && rayPayloadWrapperLayout->getSize() == rayPayloadSize, "the stage-facing RayPayload wrapper must add no lanes to its resolved transport record");
                                                             ...
                                                             nr::test::require(nr::rhi::shaderLayoutAbiEquivalent(offLayout.abiSignature(), onLayout.abiSignature()), "material FAS root policies must preserve contract layout ABI");
