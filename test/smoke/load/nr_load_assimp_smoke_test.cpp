...
[[nodiscard]] const nr::load::MaterialAsset &requireMaterial(
    const nr::load::SceneAsset &scene,
    std::string_view name)
{
    auto material = std::ranges::find(scene.materials, name, &nr::load::MaterialAsset::name);
    nr::test::require(
        material != scene.materials.end(),
        std::format("scene should contain material '{}'", name));
    return *material;
}
    ...
    nr::resource::MaterialTextureSlotSemantic semantic)
{
    ...
    nr::test::require(
        binding != material.textures.end(),
        std::format(
            "material '{}' should contain {} texture metadata",
            material.name,
            nr::resource::materialTextureSlotSemanticName(semantic)));
    return *binding;
}

[[nodiscard]] const nr::load::MeshAsset &requireMesh(
    const nr::load::SceneAsset &scene,
    std::string_view name)
{
    auto mesh = std::ranges::find(scene.meshes, name, &nr::load::MeshAsset::name);
    nr::test::require(
        mesh != scene.meshes.end(),
        std::format("scene should contain mesh '{}'", name));
    return *mesh;
}
...
[[nodiscard]] bool tangentNear(
    const nr::load::VertexAsset& vertex,
    const std::array<float, 3>& expectedDirection,
    float expectedSign,
    float tolerance = 1e-4f) noexcept
{
...
void requireGeneratedTransformedTangentMesh(
    ...
    std::string_view label)
{
    ...
    nr::test::requireEqual(
        ...
        std::size_t{6},
        std::format(
            "{} should split the two shared source vertices whose MikkTSpace corner frames differ",
            label));
    nr::test::require(
        mesh.indices[0] != mesh.indices[3] &&
            mesh.indices[2] != mesh.indices[4],
        std::format(
            "{} should not merge incompatible indexed-corner tangent frames",
            label));

    auto firstFaceCorners = std::views::iota(std::size_t{0}, std::size_t{3});
    nr::test::require(
        std::ranges::all_of(firstFaceCorners, [&](std::size_t corner) {
            return tangentNear(
                mesh.vertices[mesh.indices[corner]],
                std::array<float, 3>{0.0f, 1.0f, 0.0f},
                -1.0f);
        }),
        std::format(
            "{} should generate +Y/-1 tangents from UV1 after the swap/reflection KHR transform",
            label));

    constexpr auto secondFaceDirection =
        std::array<float, 3>{0.89442719f, 0.44721360f, 0.0f};
    auto secondFaceCorners = std::views::iota(std::size_t{3}, std::size_t{6});
    nr::test::require(
        std::ranges::all_of(secondFaceCorners, [&](std::size_t corner) {
            return tangentNear(
                mesh.vertices[mesh.indices[corner]],
                secondFaceDirection,
                1.0f);
        ...
        std::format(
            "{} should retain the second face's distinct +1 MikkTSpace handedness",
            label));
}

void requireNormalTextureTangentCase()
{
    auto scene = loadSceneCase(
        "normal texture effective-UV tangent generation",
    ...
    auto const& baseNormal =
        requireTextureBinding(baseAndClearcoat, nr::resource::MaterialTextureSlotSemantic::normal);
    ...
    nr::test::requireEqual(
        baseNormal.uvChannel,
        std::uint32_t{1},
        "base normal transform should select TEXCOORD_1");
    nr::test::require(
        almostEqual(baseNormal.transform.linear.x, 0.0f) &&
            almostEqual(baseNormal.transform.linear.y, 1.0f) &&
            almostEqual(baseNormal.transform.linear.z, 1.0f) &&
            almostEqual(baseNormal.transform.linear.w, 0.0f),
        "base normal should retain the swap/reflection KHR texture transform");
    nr::test::requireEqual(
        competingClearcoatNormal.uvChannel,
        std::uint32_t{0},
        "the competing clearcoat normal should retain its distinct UV0 mapping");
    requireGeneratedTransformedTangentMesh(
        requireMesh(scene, "GeneratedBaseNormalMesh"),
        "base-normal-priority mesh");
...
{
    auto scene = loadSceneCase(
        "KHR_texture_transform texCoord override",
    ...
    auto const &baseColor =
        requireTextureBinding(material, nr::resource::MaterialTextureSlotSemantic::baseColor);
    nr::test::requireEqual(
        baseColor.uvChannel,
        std::uint32_t{1},
        "KHR_texture_transform texCoord should override the textureInfo texCoord");
    nr::test::require(
        almostEqual(baseColor.transform.linear.x, 0.0f) &&
            almostEqual(baseColor.transform.linear.y, -0.5f) &&
            almostEqual(baseColor.transform.linear.z, 2.0f) &&
            almostEqual(baseColor.transform.linear.w, 0.0f) &&
            almostEqual(baseColor.transform.offset.x, 0.25f) &&
            almostEqual(baseColor.transform.offset.y, 0.5f),
        "base-color KHR_texture_transform should retain canonical glTF image-space affine values");

    auto const &occlusion =
        requireTextureBinding(material, nr::resource::MaterialTextureSlotSemantic::occlusion);
    nr::test::requireEqual(
        occlusion.uvChannel,
        std::uint32_t{1},
        "occlusion metadata should retain the KHR_texture_transform UV set override");
    nr::test::require(
        almostEqual(occlusion.transform.linear.x, 1.0f) &&
            almostEqual(occlusion.transform.linear.y, 0.0f) &&
            almostEqual(occlusion.transform.linear.z, 0.0f) &&
            almostEqual(occlusion.transform.linear.w, 1.0f) &&
            almostEqual(occlusion.transform.offset.x, 0.125f) &&
            almostEqual(occlusion.transform.offset.y, 0.25f),
        "occlusion metadata should transport its UV transform without implying shader behavior");

    nr::test::require(
        std::ranges::any_of(scene.meshes, [](const nr::load::MeshAsset &mesh) {
...
void requireMeshGpuInstancingCase(std::string_view label, const std::filesystem::path &relativeSourcePath)
{
    ...
    auto instanceNodes = scene.nodes | std::views::filter([](const nr::load::NodeAsset &node) {
                             return !node.meshIndices.empty();
                         });

    nr::test::requireEqual(scene.meshes.size(),
                           std::size_t{1},
                           std::format("{} should retain one shared source mesh", label));
    nr::test::requireEqual(static_cast<std::size_t>(std::ranges::distance(instanceNodes)),
    ...
    nr::test::requireEqual(sharedMeshIndex,
                           std::uint32_t{0},
                           std::format("{} instances should reference the sole imported mesh", label));
    nr::test::require(std::ranges::all_of(instanceNodes, [&](const nr::load::NodeAsset &node) {
                          return node.parentIndex == anchorIndex &&
                                 node.meshIndices.size() == 1 &&
    ...
    nr::test::requireEqual(anchor.childIndices.size(),
                           expectedInstanceCount,
    ...
    nr::test::require(hasTranslation(0.0f),
                      std::format("{} should decode the first instance translation", label));
    nr::test::require(hasTranslation(10.0f),
                      std::format("{} should decode the last instance translation", label));

    nr::test::require(std::ranges::any_of(instanceNodes, [&](const nr::load::NodeAsset &node) {
...
const nr::test::CaseRegistrar gltfTextureTransformCase{
    "load smoke imports UV1 and KHR_texture_transform metadata",
    [] {
...
const nr::test::CaseRegistrar glbMeshGpuInstancingCase{
    ...
    [] {
        requireMeshGpuInstancingCase(
            ...
            std::filesystem::path{"assets/glTF-Sample-Assets/Models/SimpleInstancing/glTF-Binary/SimpleInstancing.glb"});
    }};
} // namespace
