                                                              ...
                                                              std::uint32_t x, std::uint32_t y)
{
    ...
    auto &uploadReadback = device.uploadReadback();
    auto ticket =
        uploadReadback.readbackImage(image, vk::ImageLayout::eShaderReadOnlyOptimal, nr::rhi::QueueRole::Graphics,
...
const nr::test::CaseRegistrar defaultEnvironmentUploadCase{
    "rhi uploads the default 8K RGBA16F environment through the 128 MiB ring", [] {
        auto const sourcePath =
            std::filesystem::path{std::string{nr::projectRoot}} / "assets" / "envMap" / "studio_small_09_8k.exr";
        ...
        nr::test::require(texture.format == vk::Format::eR16G16B16A16Sfloat,
        ...
        auto &uploadReadback = device.uploadReadback();
        auto uploadTicket =
            uploadReadback.uploadImage(texture.levels.front().bytes, image, vk::ImageLayout::eUndefined,
                                       vk::ImageLayout::eShaderReadOnlyOptimal, makeEnvironmentUploadPlan(device));
        nr::test::require(uploadTicket.valid(), "default environment upload ticket should be valid");
        nr::test::require(uploadTicket.signalValue >= 2u,
        ...
        std::ranges::for_each(probes, [&](auto coordinate) {
            ...
            auto const sourceTexelIndex = static_cast<std::size_t>(y) * texture.width + x;
            auto const sourceOffset = sourceTexelIndex * bytesPerTexel;
            auto const expected =
                std::span<const std::byte>{texture.levels.front().bytes}.subspan(sourceOffset, bytesPerTexel);
            nr::test::requireEqual(readback.size(), bytesPerTexel);
            nr::test::require(
                std::ranges::equal(readback, expected),
                std::format("environment texel ({}, {}) should survive its upload-ring chunk exactly", x, y));
        });
