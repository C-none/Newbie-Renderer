...
static_assert(offsetof(RtContractVertex, position) == 0u);
static_assert(offsetof(RtContractVertex, normal) == 12u);
static_assert(offsetof(RtContractVertex, tangent) == 24u);
static_assert(offsetof(RtContractVertex, texCoord0) == 40u);
static_assert(offsetof(RtContractVertex, texCoord1) == 48u);
static_assert(offsetof(RtContractVertex, color) == 56u);
...
[[nodiscard]] std::array<RtMaterialHeader, kMaterialCount> makeMaterialHeaders()
{
    ...
    std::ranges::for_each(std::views::iota(std::uint32_t{0u}, kMaterialCount), [&](std::uint32_t materialIndex) {
        ...
        header.baseColorFactor = glm::vec4{1.0f};
        header.roughnessNormalOcclusionAlpha = glm::vec4{1.0f};
    });

    headers[1].featureFlags = RtMaterialFeatureFlag::alphaMask;
    headers[1].alphaMode = AlphaMode::mask;
    headers[1].baseColorFactor = glm::vec4{1.0f, 1.0f, 1.0f, 0.0f};
...
[[nodiscard]] std::vector<RtMaterialTextureRef> makeMaterialTextureRefs()
{
    ...
    std::ranges::for_each(refs, [](RtMaterialTextureRef &ref) {
        ref.uvLinear = glm::vec4{1.0f, 0.0f, 0.0f, 1.0f};
        ref.textureId = 0u;
        ref.uvSet = 0u;
    });
    return refs;
