...
const nr::test::CaseRegistrar maintenance9OwnershipCase{
    ...
    [] {
        ...
        auto const guideUsage = vk::ImageUsageFlagBits::eStorage |
                                vk::ImageUsageFlagBits::eSampled |
                                vk::ImageUsageFlagBits::eTransferDst |
                                vk::ImageUsageFlagBits::eTransferSrc;
        auto guideInfo = nr::rhi::makeImageCreateInfo(
            vk::Format::eR16G16B16A16Sfloat,
            vk::Extent2D{128u, 72u},
            guideUsage);
        auto guide = device.resourceFactory.createImage(
            guideInfo,
        ...
        static_cast<void>(builder.addPass(
            "Maintenance9.PathTracing.Write",
            node,
            uses,
        ...
        nr::test::requireEqual(
            unspecializedPlan.initialReleaseBatches.size(),
            std::size_t{1},
        ...
        auto prepared = executor.prepareFrame(
            std::move(compiled),
            nr::renderer::RenderGraphExecutor::ExecuteContext{
                .device = device,
            });

        nr::test::require(
            prepared.plan.initialReleaseBatches.empty(),
            "maintenance9-eligible retained guide should not create an initial ownership submit");
        nr::test::require(
            prepared.plan.batches.front().headAcquireTransitions.empty(),
            "maintenance9-eligible retained guide should not record an ownership acquire");
        nr::test::requireEqual(
            prepared.plan.batches.front().initialResourceWaits.size(),
            std::size_t{1},
            "implicit initial acquire should wait on the retained source submission");
        nr::test::requireEqual(
            prepared.plan.batches.front().initialResourceWaits.front().token,
            nr::renderer::RendererSubmitToken{
                .queue = nr::renderer::QueueDomain::Compute,
                .value = 1u,
            });
        nr::test::requireEqual(
            prepared.plan.totalInPassBarrierCount,
            std::size_t{1},
            "omitted image ownership should retain its consumer-side layout transition");
        nr::test::require(
            prepared.compiled.ownershipTransitions.empty(),
        ...
        nr::test::requireEqual(
            transition.strength,
            nr::renderer::DependencyStrength::BarrierRequired);
