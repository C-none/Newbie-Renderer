...
[[nodiscard]] bool hasFeature(const nr::scene::RtCompiledMaterial &material,
                              nr::scene::RtMaterialFeatureFlag flag) noexcept
{
...
[[nodiscard]] nr::resource::AlphaMode alphaModeFromHint(nr::load::MaterialAlphaModeHint hint) noexcept
{
    switch (hint)
    {
    case nr::load::MaterialAlphaModeHint::mask:
        return nr::resource::AlphaMode::mask;
    case nr::load::MaterialAlphaModeHint::blend:
...
[[nodiscard]] std::vector<nr::scene::RtCompiledMaterial> compileAssetMaterials(
...
void requireSourceDeclaresExtension(const std::filesystem::path &relativePath, std::string_view extension)
{
    ...
    nr::test::require(parsed.valid(),
                      std::format("asset '{}' should contain valid JSON", relativePath.generic_string()));
...
[[nodiscard]] std::size_t materialTextureSemanticCount(const nr::load::SceneAsset &scene,
    ...
    const nr::load::MaterialAsset &material, nr::resource::MaterialTextureSlotSemantic semantic)
{
    ...
    nr::test::require(bindingIt != material.textures.end(),
                      std::format("material '{}' should bind {} texture", material.name,
                                  nr::resource::materialTextureSlotSemanticName(semantic)));
    return *bindingIt;
}

[[nodiscard]] std::string textureFilename(const nr::load::SceneAsset &scene,
...
void requireTextureFilename(const nr::load::SceneAsset &scene, const nr::load::MaterialAsset &material,
                            nr::resource::MaterialTextureSlotSemantic semantic, std::string_view expectedFilename)
{
    ...
    nr::test::require(filename == expectedFilename,
                      std::format("material '{}' {} texture expected '{}', got '{}'", material.name,
                                  nr::resource::materialTextureSlotSemanticName(semantic), expectedFilename, filename));
}
...
inline constexpr auto kSponzaGltfMaterial6PrimitiveMeshes = std::array{
    std::string_view{"meshes[0]-5"},  std::string_view{"meshes[0]-8"},  std::string_view{"meshes[0]-11"},
    std::string_view{"meshes[0]-16"}, std::string_view{"meshes[0]-18"}, std::string_view{"meshes[0]-20"},
    std::string_view{"meshes[0]-22"}, std::string_view{"meshes[0]-24"}, std::string_view{"meshes[0]-26"},
    std::string_view{"meshes[0]-28"}, std::string_view{"meshes[0]-30"}, std::string_view{"meshes[0]-32"},
    std::string_view{"meshes[0]-34"}, std::string_view{"meshes[0]-36"}, std::string_view{"meshes[0]-48"},
};

[[nodiscard]] bool materialHasTextureFilename(const nr::load::SceneAsset &scene,
                                              const nr::load::MaterialAsset &material, ExpectedTextureSlot expected)
{
    auto bindingIt = std::ranges::find_if(material.textures, [&](const nr::load::MaterialTextureBinding &binding) {
    ...
    nr::test::require(bindingIt->textureIndex < scene.textures.size(),
                      std::format("material '{}' {} texture index {} should be in range", material.name,
...
[[nodiscard]] std::string materialTextureSummary(const nr::load::SceneAsset &scene, std::size_t materialIndex,
                                                 const nr::load::MaterialAsset &material)
{
    ...
    std::ranges::for_each(material.textures, [&](const nr::load::MaterialTextureBinding &binding) {
        ...
        summary += std::format(" {}={}", nr::resource::materialTextureSlotSemanticName(binding.semantic), filename);
    });
    return summary;
}
...
[[nodiscard]] std::size_t requireUniqueMaterialIndexByTextureFilenames(
    const nr::load::SceneAsset &scene, std::span<const ExpectedTextureSlot> expectedSlots)
{
    auto materialIndices = std::views::iota(std::size_t{0}, scene.materials.size());
    auto matches = materialIndices | std::views::filter([&](std::size_t materialIndex) {
    ...
    nr::test::require(!matches.empty(), std::format("expected exactly one imported material with Sponza arch textures; "
                                                    "found none. Imported material textures:{}",
                                                    materialTextureSummaries(scene)));
    nr::test::require(
        matches.size() == std::size_t{1},
        std::format(
            "expected exactly one imported material with Sponza arch textures; found {}. Imported material textures:{}",
            matches.size(), materialTextureSummaries(scene)));
    return matches.front();
}

[[nodiscard]] const nr::load::MeshAsset &requireMeshByName(const nr::load::SceneAsset &scene, std::string_view meshName)
{
    auto meshIt =
        std::ranges::find_if(scene.meshes, [&](const nr::load::MeshAsset &mesh) { return mesh.name == meshName; });
...
void requireGltfMaterial6UvOrientation(const nr::load::SceneAsset &scene)
{
    ...
    std::ranges::for_each(kSponzaGltfMaterial6PrimitiveMeshes, [&](std::string_view meshName) {
        ...
        nr::test::require(!mesh.vertices.empty(),
                          std::format("Sponza primitive mesh '{}' should contain vertices", meshName));
        auto const uvVValues = mesh.vertices | std::views::transform([](const nr::load::VertexAsset &vertex) {
...
void requireRtTextureRef(const nr::scene::RtCompiledMaterial &material,
                         nr::resource::MaterialTextureSlotSemantic semantic)
{
...
const nr::test::CaseRegistrar rtMaterialCompilerCase{
    "scene RT material compiler builds canonical layers and anisotropy ABI", [] {
        ...
        material.slot(nr::resource::MaterialTextureSlotSemantic::baseColor).texture =
            nr::resource::TextureHandle{2u, 1u};
        material.slot(nr::resource::MaterialTextureSlotSemantic::baseColor).uvSet = 0u;
        material.slot(nr::resource::MaterialTextureSlotSemantic::anisotropy).texture =
            nr::resource::TextureHandle{3u, 1u};
        ...
        nr::test::requireEqual(compiled.layers[3].aux0,
        ...
        nr::test::require(std::ranges::none_of(compiled.layers,
                                               [](const nr::scene::RtMaterialLayerRecord &layer) {
...
const nr::test::CaseRegistrar rtMaterialLayerFlagMatrixCase{
    "scene RT material layer-flag matrix covers unlit, lit combinations, dense refs and default normals", [] {
        ...
        // unlit -> layerFlags none, no layer records; authored PBR extension data is ignored.
        {
            ...
            material.slot(MaterialTextureSlotSemantic::anisotropy).texture = nr::resource::TextureHandle{2u, 1u};
        ...
        // authored texture falls back to scalar-only +T anisotropy.
        {
            ...
            nr::test::require(!hasLayer(zeroCompiled, Layer::anisotropicBaseLobe),
            ...
            nr::test::require(hasLayer(presentCompiled, Layer::anisotropicBaseLobe),
                              "texture residency must not change anisotropic base-lobe specialization");
            auto const &anisotropyRef =
                presentCompiled
                    .textureRefs[nr::resource::materialTextureSlotIndex(MaterialTextureSlotSemantic::anisotropy)];
            ...
            nr::test::require(nearlyEqual(nr::scene::compileRtMaterial(material, ids).header.anisotropy.y, 0.0f));
            material.anisotropy->rotation = std::numeric_limits<float>::infinity();
            nr::test::require(nearlyEqual(nr::scene::compileRtMaterial(material, ids).header.anisotropy.y, 0.0f));
            material.anisotropy->rotation = -0.75f;
            nr::test::require(nearlyEqual(nr::scene::compileRtMaterial(material, ids).header.anisotropy.y, -0.75f));
        }
        ...
        // Positive transmission defaults to a thin 1.5-IOR boundary.
        {
            ...
            nr::test::requireEqual(compiled.layers[1].aux0,
        ...
        // entry transports UV selection and its affine transform without a redundant slot field.
        {
            ...
            nr::test::requireEqual(
                compiled
                    ...
                    .textureId,
                0u, "absent texture slot must resolve to id 0");
...
const nr::test::CaseRegistrar rtMaterialShaderUvAndAoPolicyCase{
    "RT material shader keeps transformed nearest LOD0 FAS and anisotropy semantics", [] {
        ...
        nr::test::require(materialTypes.contains("public float2 selectMaterialUv(") &&
        ...
        nr::test::require(hitSurface.contains("kRtVertexOffsetTexCoord0 = 40u") &&
        ...
        nr::test::require(genericSamplingBody.contains("materialTextureUv(") &&
                              genericSamplingBody.contains(".SampleLevel(") && genericSamplingBody.contains("0.0f"),
                          "generic texture sampling should use transformed UVs and explicit mip 0");
        nr::test::require(materialSampling.substr(normalSample, anisotropyDecode - normalSample)
                              .contains("sampleMaterialTextureVariant("),
        ...
        nr::test::require(anisotropyDecodeBody.contains("anisotropySample.rg * 2.0f - 1.0f") &&
                          ...
                          "pure anisotropy decoding should implement RG direction, +T fallback, CCW rotation, B "
                          "strength and final-frame handedness");
        auto const anisotropySamplingBody = materialSampling.substr(anisotropySample, coreSample - anisotropySample);
        nr::test::require(anisotropySamplingBody.contains("float3(1.0f, 0.5f, 1.0f)") &&
                              anisotropySamplingBody.contains("sampleMaterialTextureVariant(") &&
                              anisotropySamplingBody.contains("decodeMaterialAnisotropy(") &&
                              !materialSampling.contains(".Sample(") && !materialSampling.contains("ddx(") &&
                              !materialSampling.contains("ddy(") && !materialSampling.contains("RayCone") &&
                              !materialSampling.contains("rayCone") && !materialSampling.contains("Wave") &&
                              !materialSampling.contains("wave") && !materialSampling.contains("WIS") &&
                              !materialSampling.contains("wis"),
                          "all RT material slots, including anisotropy, should retain nearest explicit LOD0 sampling "
                          "without derivatives, hardware anisotropy, or ray footprints");
        nr::test::require(
            materialSampling.contains("public float4 sampleMaterialTextureVariant(") &&
                materialSampling.contains("if (kEnableFilterAfterShading)") &&
                materialSampling.contains("stochasticBilinearTexelCenterUv(") &&
                materialSampling.contains("gSceneTextures[textureRef.textureId].SampleLevel(uv, 0.0f);") &&
                stochasticTextureFiltering.contains("public float2 stochasticBilinearTexelCenterUv(") &&
                stochasticTextureFiltering.contains("selectStochasticFilterUpperTap("),
            "FAS-on should stochastically select one bilinear reconstruction tap while FAS-off directly fetches one "
            "nearest LOD0 texel");
        nr::test::require(
            sceneTextureBinding.contains("sceneTextureTableNearestSamplerDesc") &&
                ...
                sceneTextureBinding.contains(".minLod = 0.0f") && sceneTextureBinding.contains(".maxLod = 0.0f") &&
                !sceneTextureBinding.contains("anisotropyEnable") && !sceneTextureBinding.contains("maxAnisotropy"),
        ...
        nr::test::require(materialBsdf.contains("public struct GgxSpecularEnergyTerms") &&
                          ...
                          "opaque base and clearcoat GGX should use UE-style correlated Smith plus analytic "
                          "Spec.E/Spec.W without applying opaque compensation to active glass");
        nr::test::require(
        ...
        nr::test::require(
            materialPayload.contains("public float3 materialPayloadFacingGeometryNormal(") &&
                ...
                materialPayload.contains("reflectedDirection - geometryCosine * facingGeometryNormal") &&
                materialPayload.contains("clippedReflection - incidentDirection"),
            "specular lobes should clip view-dependent shading normals against a view-facing geometry normal");
        nr::test::require(
            materialPayload.contains("public struct MaterialPayloadReflectionEvaluation") &&
                materialPayload.contains("public float3 materialPayloadMirrorReflectionDirection(") &&
                materialPayload.contains("public float3 materialPayloadFoldReflectionDirection(") &&
                materialPayload.contains("2.0f * dot(safeDirection, safeGeometryNormal) * safeGeometryNormal"),
            "reflection folding should use a unit-Jacobian mirror across the facing geometry plane");
        nr::test::require(
            materialPayload.contains("float3 rawNormal = safeNormalize(payload.shadingNormal, facingGeometryNormal)") &&
                materialPayload.contains("materialPayloadDiffusePdf(rawNormal, lightDirection)") &&
                materialPayload.contains("float3 specularNormal = adjustMaterialPayloadSpecularNormal("),
            "base diffuse should retain the raw shading frame while the GGX interface uses its adjusted specular "
            "frame");
        nr::test::require(
            materialPayload.contains("public float3 diffuseProjected") &&
                ...
                materialPayload.contains("scatter.diffuseWeight = bsdf.diffuseProjected / pdf") &&
                materialPayload.contains("scatter.specularWeight = bsdf.specularProjected / pdf"),
        ...
        auto const clearcoatBegin = materialPayload.find("public struct ClearcoatBsdfLobe");
        auto const resolvedEvaluationBegin =
            materialPayload.find("public ResolvedMaterialBsdfComponents evaluateResolvedMaterialBsdfComponentsVariant");
        nr::test::require(baseSurfaceBegin != std::string::npos && sheenBegin != std::string::npos &&
                              clearcoatBegin != std::string::npos && resolvedEvaluationBegin != std::string::npos &&
                              baseSurfaceBegin < sheenBegin && sheenBegin < clearcoatBegin &&
                              clearcoatBegin < resolvedEvaluationBegin,
                          "reflection lobe bodies should retain their expected material-payload order");

        auto const baseSurfaceBody = materialPayload.substr(baseSurfaceBegin, sheenBegin - baseSurfaceBegin);
        auto const sheenBody = materialPayload.substr(sheenBegin, clearcoatBegin - sheenBegin);
        auto const clearcoatBody = materialPayload.substr(clearcoatBegin, resolvedEvaluationBegin - clearcoatBegin);
        auto const lobeUsesPushForwardFold = [](std::string_view body) {
            return body.contains("evaluateFoldedReflection(") && body.contains("foldedReflectionPdf(") &&
                   body.contains("materialPayloadMirrorReflectionDirection(") &&
                   body.contains("materialPayloadFoldReflectionDirection(");
        };
        nr::test::require(
            lobeUsesPushForwardFold(baseSurfaceBody) && lobeUsesPushForwardFold(sheenBody) &&
        ...
        nr::test::require(
            materialPayload.contains("public bool materialPayloadGeometrySupportsReflection(") &&
                ...
                materialPayload.contains("public bool materialPayloadGeometrySupportsTransmission(") &&
                materialPayload.contains("return dot(facingGeometryNormal, lightDirection) <= kMaterialMinCosTheta"),
...
const nr::test::CaseRegistrar rtSampleAssetsCase{
    "scene RT material compiler covers Khronos sample asset material features", [] {
        ...
        nr::test::require(std::ranges::any_of(toyCar,
        ...
        nr::test::require(std::ranges::any_of(toyCar,
                                              [](const nr::scene::RtCompiledMaterial &material) {
                                                  return hasLayer(material,
                                                                  nr::scene::RtMaterialLayerFlag::transmission);
                                              }),
                          "ToyCar should produce at least one RT transmission material");

        auto anisotropy = compileAssetMaterials(
            "assets/glTF-Sample-Assets/Models/AnisotropyRotationTest/glTF/AnisotropyRotationTest.gltf");
        nr::test::require(std::ranges::any_of(anisotropy,
                                              [](const nr::scene::RtCompiledMaterial &material) {
        ...
        nr::test::require(std::ranges::any_of(anisotropyStrength,
                                              [](const nr::scene::RtCompiledMaterial &material) {
                                                  return material.header.anisotropy.x > 0.0f &&
        ...
        auto anisotropyDisc =
            compileAssetMaterials("assets/glTF-Sample-Assets/Models/AnisotropyDiscTest/glTF/AnisotropyDiscTest.gltf");
        nr::test::require(std::ranges::any_of(anisotropyDisc,
                                              [](const nr::scene::RtCompiledMaterial &material) {
                                                  return material.header.anisotropy.z > 0.5f;
                                              }),
                          "AnisotropyDiscTest should retain a usable anisotropy texture");

        auto compareAnisotropy =
            compileAssetMaterials("assets/glTF-Sample-Assets/Models/CompareAnisotropy/glTF/CompareAnisotropy.gltf");
        nr::test::require(std::ranges::any_of(compareAnisotropy,
                                              [](const nr::scene::RtCompiledMaterial &material) {
                                                  return material.header.anisotropy.x > 0.0f;
                                              }),
...
const nr::test::CaseRegistrar sponzaMaterialTextureImportCase{
    "Sponza glTF import preserves textured arch material slots", [] {
        ...
        nr::test::require(
            imported->materials.size() >= std::size_t{25},
            std::format("Sponza should keep at least 25 source materials, got {}", imported->materials.size()));
        nr::test::requireEqual(
        ...
        auto const alphaMaskMaterialCount =
            std::ranges::count_if(imported->materials, [](const nr::load::MaterialAsset &material) {
        ...
        std::ranges::for_each(imported->materials, [](const nr::load::MaterialAsset &material) {
            ...
            nr::test::require(material.alphaCutoff.has_value(),
                              "Sponza alpha-mask material should preserve glTF alphaCutoff");
            nr::test::require(nearlyEqual(*material.alphaCutoff, 0.5f),
                              "Sponza alpha-mask material should keep alphaCutoff=0.5");

            auto runtimeMaterial = convertImportedMaterial(material);
            auto compiled = nr::scene::compileRtMaterial(runtimeMaterial, makeTextureIds(runtimeMaterial));
            nr::test::require(hasFeature(compiled, nr::scene::RtMaterialFeatureFlag::alphaMask),
                              "Sponza alpha-mask material should compile to RT alpha-mask feature");
        });

        auto const archMaterialIndex =
            requireUniqueMaterialIndexByTextureFilenames(*imported, kSponzaArchMaterialTextureSlots);
        ...
        auto compiledArchMaterial =
            nr::scene::compileRtMaterial(archRuntimeMaterial, makeTextureIds(archRuntimeMaterial));
...
const nr::test::CaseRegistrar unsupportedExtensionAssetCase{
    "scene RT material compiler keeps unsupported glTF extensions out of canonical layers", [] {
        auto unsupportedAssets = std::array{
            std::pair{
                std::filesystem::path{
                    "assets/glTF-Sample-Assets/Models/CompareIridescence/glTF/CompareIridescence.gltf"},
                std::string_view{"KHR_materials_iridescence"},
            },
            std::pair{
                std::filesystem::path{
                    "assets/glTF-Sample-Assets/Models/DiffuseTransmissionTest/glTF/DiffuseTransmissionTest.gltf"},
        ...
        std::ranges::for_each(unsupportedAssets, [](const auto &asset) {
            ...
            nr::test::require(
                std::ranges::all_of(compiled,
                                    [](const nr::scene::RtCompiledMaterial &material) {
                                        return std::ranges::all_of(
                                            material.layers, [](const nr::scene::RtMaterialLayerRecord &layer) {
                                                return layer.layer <= nr::scene::RtMaterialLayerFlag::transmission;
                                            });
                                    }),
