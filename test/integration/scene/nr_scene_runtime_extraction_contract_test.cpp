...
[[nodiscard]] nr::load::SceneAsset makeRuntimeSceneAsset(bool includeCamera)
{
    ...
    scene.sourcePath = includeCamera ? std::filesystem::path{"runtime_camera_scene.gltf"} : std::filesystem::path{"runtime_readiness_scene.gltf"};
...
const nr::test::CaseRegistrar materialSemanticClassificationCase{"scene material bridge preserves Assimp PBR texture semantics", [] {
                                                                     ...
                                                                     nr::test::require(material.slot(nr::resource::MaterialTextureSlotSemantic::baseColor).texture == textureHandles[0], "base_color should bind baseColor slot");
                                                                     nr::test::require(material.slot(nr::resource::MaterialTextureSlotSemantic::normal).texture == textureHandles[1], "normal_camera should bind normal slot");
                                                                     ...
                                                                     auto extensionMaterialHandle = scene.findMaterialHandleByStableKey(nr::scene::SceneBridge::makeMaterialCanonicalKey(sceneAsset, 1));
                                                                     ...
                                                                     nr::test::require(nr::resource::hasAnyFeature(extensionFlags, nr::resource::MaterialFeatureFlag::sheen), "extension flags should include sheen");
                                                                     nr::test::require(nr::resource::hasAnyFeature(extensionFlags, nr::resource::MaterialFeatureFlag::transmission), "extension flags should include transmission");
                                                                     nr::test::require(nr::resource::hasAnyFeature(extensionFlags, nr::resource::MaterialFeatureFlag::anisotropy), "extension flags should include anisotropy");

                                                                     auto const volumeThicknessBound = std::ranges::any_of(extensionMaterial.textureSlots, [&](const nr::resource::MaterialTextureSlot &slot) { return slot.texture == textureHandles[9]; });
...
{
    auto requiredSelection = domain == nr::scene::ScenePacketDomain::rasterDraw ? nr::scene::sceneSelectionMask(nr::scene::SceneSelectionBit::rasterOpaque) : nr::scene::sceneSelectionMask(nr::scene::SceneSelectionBit::rtMain);
...
[[nodiscard]] RuntimeHandles resolveRuntimeHandles(const nr::scene::Scene &scene, const nr::load::SceneAsset &sceneAsset)
{
...
const nr::test::CaseRegistrar cameraAspectCase{"scene primary camera uses authored aspect and viewport overrides", [] {
                                                   nr::rhi::Device device{};
                                                   auto scene = nr::scene::Scene(nr::scene::SceneCreateInfo{.device = device});
                                                   ...
                                                   nr::test::require(almostEqual(projectionAspectRatio(authoredCamera->projection), 1.0f, 1e-3f), "authored aspect should be used without viewport");
                                                   nr::test::require(almostEqual(projectionAspectRatio(squareCamera->projection), 1.0f, 1e-3f), "square viewport should produce 1:1 projection aspect");
                                                   nr::test::require(almostEqual(projectionAspectRatio(wideCamera->projection), 16.0f / 9.0f, 1e-3f), "wide viewport should produce 16:9 projection aspect");
                                               }};

const nr::test::CaseRegistrar activeInstanceCase{"scene extraction filters inactive instances", [] {
                                                     nr::rhi::Device device{};
                                                     auto scene = nr::scene::Scene(nr::scene::SceneCreateInfo{.device = device});
                                                     ...
                                                     nr::test::require(packets.revisions.rt.get<nr::scene::SceneRtRevisionDomain::topology>() != initialRevisions.rt.get<nr::scene::SceneRtRevisionDomain::topology>(), "template and instance lifecycle should advance topology revision");
                                                     ...
                                                     nr::test::require(afterDestroy.revisions.rt.get<nr::scene::SceneRtRevisionDomain::topology>() != packets.revisions.rt.get<nr::scene::SceneRtRevisionDomain::topology>(), "destroying an instance should advance topology revision");
                                                     nr::test::require(afterDestroy.rasterDraws.empty(), "destroying active instance should remove raster packets");
                                                 }};

const nr::test::CaseRegistrar lightRuntimePacketCase{"scene extraction emits active directional point and spot light packets", [] {
                                                         nr::rhi::Device device{};
                                                         auto scene = nr::scene::Scene(nr::scene::SceneCreateInfo{.device = device});
                                                         ...
                                                         nr::test::require(almostEqual(spotRecord->get().cpu.outerConeRadians, glm::radians(25.0f)), "spot outer cone should be preserved");
                                                         ...
                                                         auto packetFor = [&](nr::resource::LightAssetHandle handle) -> const nr::scene::SceneLightPacket * {
                                                             auto found = std::ranges::find_if(packets.lights, [&](const nr::scene::SceneLightPacket &packet) { return packet.light == handle; });
...
const nr::test::CaseRegistrar sceneLightAliasGpuAbiCase{"scene light alias table gpu abi and energy weighting are stable", [] {
                                                            ...
                                                            nr::test::require(almostEqual(nr::scene::sceneLightAliasEnergy(glm::vec3{1.0f}, 2.0f), 2.0f), "white light alias energy should equal intensity");
                                                            nr::test::require(almostEqual(nr::scene::sceneLightAliasEnergy(glm::vec3{0.0f, 1.0f, 0.0f}, 2.0f), 1.4304f), "green light alias energy should use Rec.709 luminance");
                                                            nr::test::require(almostEqual(nr::scene::sceneLightAliasEnergy(glm::vec3{1.0f}, -2.0f), 0.0f), "negative light intensity should have zero alias energy");
...
const nr::test::CaseRegistrar readinessCase{"scene extraction applies domain-specific residency readiness", [] {
                                                nr::rhi::Device device{};
                                                auto scene = nr::scene::Scene(nr::scene::SceneCreateInfo{.device = device});
                                                auto sceneAsset = makeRuntimeSceneAsset(false);
