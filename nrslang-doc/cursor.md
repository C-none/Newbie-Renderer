...
At this point a reader may be left asking where this approach actually bottoms out; it can’t be structs all the way down.
...
The addBindingsFrom() method needs to look at the kind of type that is passed in and add zero or more entries to m_bindings, possibly via recursively calling itself. It can do this by switching on the kind of type passed in:
