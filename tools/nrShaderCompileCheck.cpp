...
int main(int argc, char **argv)
{
    ...
    auto sourcePaths =
        ...
        std::views::transform([](const char* value) {
            return std::filesystem::path{value};
        }) |
        std::ranges::to<std::vector>();
    if (std::ranges::any_of(sourcePaths, [](const auto& sourcePath) {
            return sourcePath.empty() || sourcePath.is_absolute();
        }))
    {
    ...
    auto requests = sourcePaths |
                    std::views::transform([](const auto& sourcePath) {
    ...
    if (programs.size() != requests.size() ||
