...
RT material shading declarations live in `shader/include/material`. The RT material layer set is a `[Flags] RtMaterialLayerFlag` combined mask (`none` == unlit; non-zero masks always contain `baseSurface` plus any subset of clearcoat/sheen/transmission). `RtBaseLobeVariant` is an independent compile-time dimension: unlit is isotropic-only, while all eight lit layer masks have isotropic and anisotropic forms. `payload.slang` owns the two-dimension specialized resolvers and BSDF variants; optional lobes remain inside static layer-flag blocks, while `BaseGgxDistribution<BaseLobeVariant>` shares the base reflection/thin-transmission/volume-transmission GGX evaluation, PDF, and VNDF sampling. `pathTracing/chs.slang` exposes `MaterialCHS<LayerFlags, BaseLobeVariant>`, which the PathTracing node binds through the link-time `CHS` alias. Anisotropy texture presence, UV selection, affine transform, RG direction, B strength, and scalar rotation remain runtime material data; texture reads keep the explicit `SampleLevel` path and clearcoat remains isotropic.
