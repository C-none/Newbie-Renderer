...
- GPU-only uploads use the RHI `UploadReadbackContext` ring. Do not add persistent per-node `TransferSrc` staging buffers; direct mapped writes remain valid only for intentionally CPU-writable frame resources.
