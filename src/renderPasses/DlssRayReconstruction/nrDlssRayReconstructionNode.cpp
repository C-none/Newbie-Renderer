...
void validateDlssOptimalSettings(const nr::rhi::DlssOptimalSettings &settings, nr::rhi::DlssDimensions targetSize, nr::rhi::DlssQuality quality)
{
    nr::nrAssert(settings.status.success(), std::format("DLSS RR optimal-settings query failed: {}", settings.status.message));
    nr::nrAssert(settings.optimalRenderSize.valid() && settings.minimumRenderSize.valid() && settings.maximumRenderSize.valid(), "DLSS RR optimal-settings query returned zero dimensions.");
    nr::nrAssert(settings.minimumRenderSize.width <= settings.optimalRenderSize.width && settings.minimumRenderSize.height <= settings.optimalRenderSize.height && settings.optimalRenderSize.width <= settings.maximumRenderSize.width &&
                     settings.optimalRenderSize.height <= settings.maximumRenderSize.height && settings.maximumRenderSize.width <= targetSize.width && settings.maximumRenderSize.height <= targetSize.height,
    ...
    {
        nr::nrAssert(settings.optimalRenderSize == targetSize, "DLSS RR DLAA optimal render size must equal the target size.");
    }
}
...
{
DlssRayReconstructionResolutionController::DlssRayReconstructionResolutionController() : impl_(std::make_unique<detail::DlssRayReconstructionResolutionControllerImpl>())
{
}
...
nr::renderer::FrameResolutionPlan DlssRayReconstructionResolutionController::resolve(DlssRayReconstructionResolutionRequest request, vk::Extent2D displayExtent, const OptimalSettingsQuery &optimalSettingsQuery)
{
    ...
    if (request.enabled)
    {
        ...
        {
            cachedSettings = optimalSettingsQuery(nr::rhi::DlssDimensions{displayExtent.width, displayExtent.height}, request.quality);
        }
    ...
    if (impl_->snapshot.has_value())
    {
        ...
        plan.resetHistory = previous.request.enabled != request.enabled || previous.request.quality != request.quality || previous.displayExtent != plan.displayExtent || previous.renderExtent != plan.renderExtent;
    }
    else
    {
...
[[nodiscard]] std::optional<nr::renderer::NodeRuntime::StructuralSnapshot> DlssRayReconstructionNode::structuralSnapshot(const NodeFrameParameters &frameParameters) const
{
    auto branch = std::format("{};bypass={};alpha={};hdr={};debug={};quality={};roughness={};outSubrects={};overrideRender={}:{}x{};overrideTarget={}:{}x{};render={}x{};display={}x{};colorFmt={};alphaFmt={};colorKey={};alphaKey={}", input.enabled ? "enabled" : "disabled", input.bypass ? 1u : 0u,
                              ...
                              frameParameters.resolutionPlan.renderExtent.width, frameParameters.resolutionPlan.renderExtent.height, frameParameters.resolutionPlan.displayExtent.width, frameParameters.resolutionPlan.displayExtent.height, static_cast<std::uint32_t>(input.outputColorFormat),
    ...
    std::ranges::for_each(slots, [&](std::size_t index) { branch += std::format(";{}:{}:{}", index, input.includeResources[index] ? 1u : 0u, input.resourceKeys[index]); });
    std::ranges::for_each(input.create.presets, [&](auto preset) { branch += std::format(";preset={}", static_cast<std::uint32_t>(preset)); });
    std::ranges::for_each(input.evaluate.subrectBases, [&](auto base) { branch += std::format(";subrect={},{}", base.x, base.y); });
...
bool DlssRayReconstructionNode::materializeRenderGraphSkeleton(nr::renderer::RenderGraphSkeletonPatchContext &context, const NodeFrameParameters &frameParameters, const StructuralSnapshot &snapshot)
{
    ...
    auto resolutionSnapshot = resolutionController ? resolutionController->snapshot() : std::optional<DlssRayReconstructionResolutionSnapshot>{};
    if (resolutionController)
    {
        ...
        nrAssert(resolutionSnapshot->displayExtent == frameParameters.resolutionPlan.displayExtent && resolutionSnapshot->renderExtent == frameParameters.resolutionPlan.renderExtent, "DLSS RR Skeleton resolution snapshot does not match the frame plan.");
    }
    ...
    if (std::ranges::any_of(indices, [&](std::size_t index) {
            auto const slot = static_cast<nr::rhi::DlssRayReconstructionResourceSlot>(index);
            return dlssRayReconstructionResourceRequired(slot, input.create.roughnessMode, input.create.flags.alphaUpscaling) && slot != nr::rhi::DlssRayReconstructionResourceSlot::Output && slot != nr::rhi::DlssRayReconstructionResourceSlot::OutputAlpha && !handles[index].valid();
        }))
    {
        return false;
    }
    ...
    {
        nrAssert(createDesc.renderSize == createDesc.targetSize, "DLSS RR DLAA requires equal render and target sizes.");
    }
    auto const outputExtent = detail::outputExtent(createDesc.targetSize, input.evaluate.subrectBases[detail::subrectSlotIndex(nr::rhi::DlssRayReconstructionSubrectSlot::Output)], createDesc.enableOutputSubrects, "color");
    ...
    {
        auto const alphaExtent = detail::outputExtent(createDesc.targetSize, input.evaluate.subrectBases[detail::subrectSlotIndex(nr::rhi::DlssRayReconstructionSubrectSlot::OutputAlpha)], createDesc.enableOutputSubrects, "alpha");
    ...
    nrAssert(hasReflectionMotionVectors || (handles[specularHitIndex].valid() && (input.evaluate.automaticMatrices || hasManualMatrices)), "DLSS RR requires reflection motion vectors, or hit distance plus matrices.");
    ...
    auto const measuredDelta = previousBuildTime_ == std::chrono::steady_clock::time_point{} ? 1000.0f / 60.0f : std::chrono::duration<float, std::milli>(now - previousBuildTime_).count();
    ...
    auto const coordinatedOptimalSettings = resolutionSnapshot.has_value() ? resolutionSnapshot->optimalSettings : std::nullopt;
    auto prepare = [runtime = runtime_, createDesc, coordinatedOptimalSettings](const nr::renderer::PassPrepareContext &prepareContext) {
        ...
        {
            runtime->optimalSettings = coordinatedOptimalSettings.has_value() ? *coordinatedOptimalSettings : device.dlssContext()->optimalSettings(createDesc.targetSize, createDesc.quality);
            ...
            runtime->status = std::format("Ready: render {}x{}, target {}x{}, quality {}.", createDesc.renderSize.width, createDesc.renderSize.height, createDesc.targetSize.width, createDesc.targetSize.height, nr::rhi::dlssQualityName(createDesc.quality));
        }
    };
    auto record = [runtime = runtime_, handles, descriptions, evalDesc](const nr::renderer::PassRecordContext &recordContext) mutable {
        ...
        nrAssert(static_cast<bool>(recordContext.resolveImage), "DLSS RR Skeleton record requires the graph image resolver.");
        auto const indices = std::views::iota(std::size_t{0u}, nr::rhi::kDlssRayReconstructionResourceSlotCount);
        std::ranges::for_each(indices, [&](std::size_t index) {
    ...
    if (input.evaluate.visualizeMotionVectors)
    {
        ...
        auto const motionVectorSize = detail::requiredSubrectDimensions(nr::rhi::DlssRayReconstructionResourceSlot::MotionVectors, createDesc);
        auto const outputBase = createDesc.enableOutputSubrects ? input.evaluate.subrectBases[detail::subrectSlotIndex(nr::rhi::DlssRayReconstructionSubrectSlot::Output)] : nr::rhi::DlssCoordinates{};
        auto const motionVectorBase = input.evaluate.subrectBases[detail::subrectSlotIndex(nr::rhi::DlssRayReconstructionSubrectSlot::MotionVectors)];
        ...
        auto patch = nr::renderer::ComputePassPatchBuilder{context, 1u, "DLSS.RayReconstruction.VisualizeMotionVectors", runtime_->motionVectorDebugPipeline};
        patch.sampledImage("gMotionVectors", handles[motionVectorIndex], "DLSS.RR.MotionVectors")
            ...
            .record([outputExtent](const nr::renderer::ComputePassRecordContext &computeContext) {
                ...
                computeContext.commandBuffer.dispatch(detail::dlssMotionVectorDebugDivideRoundUp(outputExtent.width, threadGroupSize), detail::dlssMotionVectorDebugDivideRoundUp(outputExtent.height, threadGroupSize), 1u);
            });
        patch.patch();
    }
    return true;
...
void DlssRayReconstructionNode::materializeCurrentFrame(NodeBuildContext &context, const NodeFrameParameters &frameParameters)
{
    ...
    if (resolutionSnapshot.has_value())
    {
        ...
        nrAssert(descriptions[colorIndex]->extent.width == createDesc.renderSize.width && descriptions[colorIndex]->extent.height == createDesc.renderSize.height, "Coordinated DLSS RR input color extent does not match the renderer render extent.");
    ...
    if (input.evaluate.visualizeMotionVectors)
    {
        ...
        auto const motionVectorSize = detail::requiredSubrectDimensions(nr::rhi::DlssRayReconstructionResourceSlot::MotionVectors, createDesc);
        auto const outputBase = createDesc.enableOutputSubrects ? input.evaluate.subrectBases[outputSubrectIndex] : nr::rhi::DlssCoordinates{};
        ...
        auto debugPass = nr::renderer::ComputePassBuilder{context, "DLSS.RayReconstruction.VisualizeMotionVectors", runtime_->motionVectorDebugPipeline};
        debugPass.sampledImage("gMotionVectors", handles[motionVectorIndex], "DLSS.RR.MotionVectors")
            ...
            .record([outputColorExtent](const nr::renderer::ComputePassRecordContext &computeContext) {
                constexpr auto threadGroupSize = 8u;
                computeContext.commandBuffer.dispatch(detail::dlssMotionVectorDebugDivideRoundUp(outputColorExtent.width, threadGroupSize), detail::dlssMotionVectorDebugDivideRoundUp(outputColorExtent.height, threadGroupSize), 1u);
            });
