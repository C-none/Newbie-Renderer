...
- RayTraceInstanceHash Node targets ray tracing work, consumes `frameResource::sceneTlas`, writes one hash color per pixel from closest-hit instance ID, and publishes `frameResource::presentSourceColor`.
...
- NormalBuffer Node targets graphics queue output, outputs world-space interpolated vertex normals for any scene, binds the scene geometry atlas once per parallel record chunk, and uses `RasterPassBuilder::recordParallel(...)` to record independent contiguous ranges of scene raster draws.
