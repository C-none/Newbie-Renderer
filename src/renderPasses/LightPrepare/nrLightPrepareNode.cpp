...
bool LightPrepareNode::materializeRenderGraphSkeleton(
    nr::renderer::RenderGraphSkeletonPatchContext& context,
    const NodeFrameParameters& frameParameters,
    const StructuralSnapshot&)
{
    nr::nrAssert(static_cast<bool>(runtime_) && device_.has_value(), "LightPrepare Skeleton patch requires initialized state.");
    ...
    detail::ensureLightBuffer(device_->get(), slot, static_cast<std::uint32_t>(frameData.records.size()), input.initialLightCapacity, frameSlot);
    detail::ensureAliasBuffer(device_->get(), slot, static_cast<std::uint32_t>(frameData.aliasRecords.size()), input.initialLightCapacity, frameSlot);

    auto patchBuffer = [&](std::size_t resourceSlot, const nr::rhi::Buffer& buffer, std::string debugName, nr::renderer::BufferUsageIntent usage) {
    ...
    patchBuffer(0u, slot.headerBuffer, std::format("LightPrepare.Header[{}]", frameSlot), nr::renderer::BufferUsageIntent::Uniform);
    patchBuffer(1u, slot.lightBuffer, std::format("LightPrepare.Records[{}]", frameSlot), nr::renderer::BufferUsageIntent::StorageRead);
    patchBuffer(2u, slot.aliasBuffer, std::format("LightPrepare.AliasTable[{}]", frameSlot), nr::renderer::BufferUsageIntent::StorageRead);
    context.patchFrameData(0u, "LightPrepare.UploadData", std::make_any<detail::LightPrepareFrameData>(std::move(frameData)));
    ...
    context.patchPass(
        ...
        [runtime = runtime_, frameDataHandle](const nr::renderer::PassPrepareContext& prepareContext) {
            ...
            frameSlotState.lightBuffer.writeMappedAndFlush(std::span<const nr::scene::SceneLightGpuRecord>{data.records.data(), data.records.size()});
            frameSlotState.aliasBuffer.writeMappedAndFlush(std::span<const nr::scene::SceneLightAliasGpuRecord>{data.aliasRecords.data(), data.aliasRecords.size()});
        },
