...
void ensureHeaderBuffer(nr::rhi::Device &device, LightPrepareFrameSlot &slot, std::size_t frameSlot)
{
    ...
    slot.headerBuffer = device.resourceFactory.createBuffer(
        nr::rhi::makeBufferCreateInfo(sizeof(nr::scene::SceneLightGpuHeader), vk::BufferUsageFlagBits::eUniformBuffer),
        nr::rhi::MemoryUsage::CpuToGpu, std::format("LightPrepare.Header[{}]", frameSlot));
    nr::nrAssert(slot.headerBuffer.valid(), "LightPrepareNode failed to create scene light header buffer.");
}

void ensureLightBuffer(nr::rhi::Device &device, LightPrepareFrameSlot &slot, std::uint32_t requiredRecordCount,
                       std::uint32_t initialCapacity, std::size_t frameSlot)
{
    ...
    slot.lightBuffer = device.resourceFactory.createBuffer(
        nr::rhi::makeBufferCreateInfo(static_cast<vk::DeviceSize>(capacity) * sizeof(nr::scene::SceneLightGpuRecord),
                                      vk::BufferUsageFlagBits::eStorageBuffer),
        nr::rhi::MemoryUsage::CpuToGpu, std::format("LightPrepare.Records[{}]", frameSlot));
...
void ensureAliasBuffer(nr::rhi::Device &device, LightPrepareFrameSlot &slot, std::uint32_t requiredRecordCount,
                       std::uint32_t initialCapacity, std::size_t frameSlot)
{
    requiredRecordCount = std::max(requiredRecordCount, 1u);
    auto const desiredCapacity = std::max(requiredRecordCount, std::max(initialCapacity, 1u));
    ...
    slot.aliasBuffer = device.resourceFactory.createBuffer(
        nr::rhi::makeBufferCreateInfo(static_cast<vk::DeviceSize>(capacity) *
                                          sizeof(nr::scene::SceneLightAliasGpuRecord),
                                      vk::BufferUsageFlagBits::eStorageBuffer),
        nr::rhi::MemoryUsage::CpuToGpu, std::format("LightPrepare.AliasTable[{}]", frameSlot));
...
[[nodiscard]] nr::scene::SceneLightGpuRecord packLightRecord(const nr::resource::LightAsset &light,
                                                             const nr::scene::SceneLightPacket &packet)
{
    auto const innerCone =
        std::isfinite(light.innerConeRadians) && light.innerConeRadians >= 0.0f ? light.innerConeRadians : 0.0f;
    ...
    auto const hasFiniteRange =
        light.type != nr::resource::LightType::directional && std::isfinite(light.range) && light.range > 0.0f;
...
[[nodiscard]] LightPrepareFrameData makeFrameData(std::size_t frameSlot,
                                                  std::vector<nr::scene::SceneLightGpuRecord> records)
{
    nr::nrAssert(records.size() <= static_cast<std::size_t>(std::numeric_limits<std::uint32_t>::max()),
...
void LightPrepareNode::materializeCurrentFrame(NodeBuildContext &context, const NodeFrameParameters &frameParameters)
{
    ...
    auto const headerResource =
        context.importBuffer(slot.headerBuffer, std::format("LightPrepare.Header[{}]", frameSlot),
    ...
    auto const lightsResource =
        context.importBuffer(slot.lightBuffer, std::format("LightPrepare.Records[{}]", frameSlot),
                             nr::renderer::ResourceLifetime::FrameLocal,
                             {
                                 nr::renderer::BufferUsageIntent::StorageRead,
                             },
                             nr::renderer::ownershipDomainFromQueue(context.queue));
    auto const aliasResource =
        context.importBuffer(slot.aliasBuffer, std::format("LightPrepare.AliasTable[{}]", frameSlot),
                             nr::renderer::ResourceLifetime::FrameLocal,
                             {
                                 nr::renderer::BufferUsageIntent::StorageRead,
                             },
    ...
    auto const frameDataHandle = context.importFrameData("LightPrepare.UploadData", std::move(frameData));
    ...
    [[maybe_unused]] auto uploadPass = context.addPass(
        ...
        "LightPrepare.Upload", [](const nr::renderer::PassRecordContext &) {},
