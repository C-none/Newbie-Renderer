...
[[nodiscard]] std::string turboJpegError(tjhandle handle)
{
...
[[nodiscard]] std::expected<Image, std::string> decodeWithTurboJpeg(std::span<const std::byte> encodedBytes)
{
    if (encodedBytes.empty())
    {
...
[[nodiscard]] std::expected<Image, std::string> decodeEmbeddedRaw(const EmbeddedRawTexture &raw)
{
