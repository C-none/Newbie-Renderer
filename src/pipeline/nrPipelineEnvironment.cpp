...
inline constexpr auto defaultEnvironmentName = std::string_view{"kloofendal_48d_partly_cloudy_puresky_8k"};
...
[[nodiscard]] bool isEnvironmentMapName(std::string_view name)
{
    ...
    return !path.has_root_name() && !path.has_root_directory() && path == path.filename() && path != "." &&
           path != "..";
}

[[nodiscard]] std::expected<EnvironmentMapAsset, std::string> resolveEnvironmentMapAsset(std::string_view name)
{
    if (!isEnvironmentMapName(name))
    {
        return std::unexpected(std::format("Invalid environment map name: '{}'.", name));
    }

    auto pathError = std::error_code{};
    auto const assetDirectory = std::filesystem::canonical(environmentMapAssetDirectoryPath(), pathError);
    if (pathError)
    {
        return std::unexpected(std::format("Failed to resolve environment map asset directory '{}': {}",
                                           environmentMapAssetDirectoryPath().generic_string(), pathError.message()));
    }

    auto const requestedSourcePath = assetDirectory / std::filesystem::path{std::format("{}.exr", name)};
    auto const resolvedSourcePath = std::filesystem::canonical(requestedSourcePath, pathError);
    if (pathError)
    {
        return std::unexpected(std::format("Failed to resolve environment map '{}': {}", name, pathError.message()));
    }
    ...
    if (pathError || !sourceIsRegularFile)
    {
        return std::unexpected(
            std::format("Environment map asset is not a regular file: '{}'.", resolvedSourcePath.generic_string()));
    }

    auto const sourceParentMatchesAssetDirectory =
        std::filesystem::equivalent(resolvedSourcePath.parent_path(), assetDirectory, pathError);
    if (pathError || !sourceParentMatchesAssetDirectory)
    {
        return std::unexpected(std::format("Environment maps must be direct files under '{}': '{}'.",
                                           assetDirectory.generic_string(), name));
    }
...
[[nodiscard]] std::expected<std::vector<std::string>, std::string> discoverEnvironmentMapNames()
{
    ...
    auto iterator = std::filesystem::directory_iterator{
        assetDirectory, std::filesystem::directory_options::skip_permission_denied, iterationError};
    if (iterationError)
    {
        return std::unexpected(std::format("Failed to enumerate environment map assets under '{}': {}",
                                           assetDirectory.generic_string(), iterationError.message()));
    }
    ...
    while (iterator != end)
    {
        ...
        if (statusError)
        {
            return std::unexpected(std::format("Failed to inspect environment map asset '{}': {}",
                                               iterator->path().generic_string(), statusError.message()));
        }

        if (std::filesystem::is_regular_file(status) && isSupportedEnvironmentMapAsset(iterator->path()))
        {
            auto const name = iterator->path().stem().string();
            if (!isEnvironmentMapName(name))
            {
                return std::unexpected(std::format("Environment map asset has an invalid selectable name: '{}'.",
        ...
        if (iterationError)
        {
            return std::unexpected(std::format("Failed while enumerating environment map assets under '{}': {}",
                                               assetDirectory.generic_string(), iterationError.message()));
        }
    }
    ...
    {
        return std::unexpected(
            std::format("Environment map asset names must be unique after removing '.exr': '{}'.", *duplicate));
    }
    return names;
}
} // namespace nr::pipeline
    ...
    std::string_view environmentMapName)
{
    ...
    if (!result)
    {
        return std::unexpected(std::format("Failed to load environment map '{}': {}",
