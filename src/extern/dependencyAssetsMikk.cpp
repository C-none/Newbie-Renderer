    ...
    std::span<Tangent> tangents)
{
    ...
    if (faceVertexCounts.empty() ||
        faceVertexCounts.size() > intMax ||
        corners.size() != tangents.size() ||
        std::ranges::any_of(faceVertexCounts, [](std::uint32_t count) {
            return count < 3u || static_cast<std::size_t>(count) > intMax;
        }))
    {
        return false;
    }
    ...
    std::exclusive_scan(
        faceVertexCounts.begin(),
        faceVertexCounts.end(),
        faceOffsets.begin(),
        std::size_t{0});
    auto const cornerCount = std::accumulate(
        faceVertexCounts.begin(),
        faceVertexCounts.end(),
        std::size_t{0});
