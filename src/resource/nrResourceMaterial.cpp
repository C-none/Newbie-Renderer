...
[[nodiscard]] std::string_view materialTextureSlotSemanticName(MaterialTextureSlotSemantic semantic) noexcept
{
    switch (semantic)
    {
    case MaterialTextureSlotSemantic::baseColor:
        return "baseColor";
    case MaterialTextureSlotSemantic::normal:
        return "normal";
    case MaterialTextureSlotSemantic::metallicRoughness:
        return "metallicRoughness";
    case MaterialTextureSlotSemantic::occlusion:
        return "occlusion";
    case MaterialTextureSlotSemantic::emissive:
        return "emissive";
    case MaterialTextureSlotSemantic::clearcoat:
        return "clearcoat";
    case MaterialTextureSlotSemantic::clearcoatRoughness:
        return "clearcoatRoughness";
    case MaterialTextureSlotSemantic::clearcoatNormal:
        return "clearcoatNormal";
    case MaterialTextureSlotSemantic::sheenColor:
        return "sheenColor";
    case MaterialTextureSlotSemantic::sheenRoughness:
        return "sheenRoughness";
    case MaterialTextureSlotSemantic::transmission:
        return "transmission";
...
{
    return std::ranges::any_of(levels, [](const ImageLevel &level) { return !level.bytes.empty(); });
}

[[nodiscard]] std::size_t Texture::byteSize() const noexcept
{
    return std::ranges::fold_left(levels, std::size_t{0},
...
[[nodiscard]] MaterialFeatureFlag Material::featureFlags() const noexcept
{
    ...
    if (transmission.has_value() && std::isfinite(transmission->factor) && transmission->factor > 0.0f)
    {
...
{
    return transmission.has_value() && std::isfinite(transmission->factor) && transmission->factor > 0.0f &&
           volumeBoundary.has_value() && std::isfinite(volumeBoundary->thicknessFactor) &&
