...
class CursorPipelineLayout
{
  public:
    ...
    void bindDescriptorSet(const vk::raii::CommandBuffer &commandBuffer, vk::PipelineBindPoint bindPoint,
                           const ShaderBindingSet &set, std::span<const std::uint32_t> dynamicOffsets = {}) const;
    ...
    void pushConstants(const vk::raii::CommandBuffer &commandBuffer, const ShaderCursor &cursor,
...
class RayTracingPipeline
{
  public:
    ...
    [[nodiscard]] std::vector<std::uint8_t> shaderGroupHandles(std::uint32_t firstGroup, std::uint32_t groupCount,
                                                               std::uint32_t handleSize) const;

    [[nodiscard]] std::vector<std::uint8_t> captureReplayShaderGroupHandles(
...
class PipelineService
{
  public:
    ...
    [[nodiscard]] ShaderBindingPool createBindingPool(const ShaderDescriptorLayout &descriptorLayout,
                                                      ShaderBindingPoolConfig config = {}) const;

    [[nodiscard]] ShaderBindingSet allocateBindingSet(
    ...
    [[nodiscard]] std::vector<ShaderBindingSet> allocateBindingSets(const CursorPipelineLayout &layout,
  ...
  private:
    ...
    [[nodiscard]] PipelineState<TPipeline> makePipelineState(const SlangProgram &slangProgram,
                                                             PipelineLayoutBundle bundle,
                                                             std::uint32_t descriptorMaxSets, TPipeline pipeline) const
    {
        auto bindingPool =
            createBindingPool(bundle.descriptorLayout, ShaderBindingPoolConfig{.maxSets = descriptorMaxSets});
