...
[[nodiscard]] std::int64_t overlapArea(const nr::rhi::WindowBounds &window, const MonitorArea &monitor) noexcept
{
    ...
    auto const overlapWidth =
        std::max<std::int64_t>(0, std::min(windowRight, monitorRight) - std::max(windowLeft, monitorLeft));
    auto const overlapHeight =
        std::max<std::int64_t>(0, std::min(windowBottom, monitorBottom) - std::max(windowTop, monitorTop));
...
[[nodiscard]] std::vector<MonitorArea> enumerateMonitorAreas()
{
    ...
    return monitors | std::views::transform([](GLFWmonitor *monitor) { return makeMonitorArea(monitor); }) |
           std::views::filter(validMonitorArea) | std::ranges::to<std::vector>();
}
...
[[nodiscard]] MonitorArea selectTargetMonitorArea(GLFWwindow *window)
{
    ...
    auto bestMonitor = std::ranges::max_element(
        monitorAreas, {}, [&](const MonitorArea &area) { return overlapArea(windowBounds, area); });
                                      ...
                                      vk::Extent2D initialExtent)
{
    ...
    result.handle.reset(glfw::createWindow(static_cast<int>(initialExtent.width),
                                           static_cast<int>(initialExtent.height), std::string(windowTitle).c_str(),
                                           nullptr, nullptr));
...
void Surface::setFullscreen(bool enabled)
{
    ...
    if (enabled)
    {
        ...
        glfwSetWindowMonitor(handle.get(), monitorArea.monitor, 0, 0, monitorArea.width, monitorArea.height,
                             monitorArea.refreshRate);
    }
    else
    {
        glfwSetWindowMonitor(handle.get(), nullptr, savedWindowedBounds_.x, savedWindowedBounds_.y,
                             sanitizeWindowDimension(savedWindowedBounds_.width),
                             sanitizeWindowDimension(savedWindowedBounds_.height), 0);
    }
