...
{
    std::reference_wrapper<const Buffer> dataBuffer;
    std::optional<std::reference_wrapper<const Buffer>> indexBuffer{};
    std::optional<std::reference_wrapper<const Buffer>> transformBuffer{};
    vk::AccelerationStructureGeometryKHR geometry{};
    vk::AccelerationStructureBuildRangeInfoKHR range{};
};

struct BlasTriangleGeometryBuffers
{
    std::reference_wrapper<const Buffer> vertex;
    std::optional<std::reference_wrapper<const Buffer>> index{};
    std::optional<std::reference_wrapper<const Buffer>> transform{};
};
...
class AccelerationStructureResource
{
  public:
    ...
    [[nodiscard]] vk::Buffer storageBufferHandle() const noexcept;

    [[nodiscard]] vk::SharingMode storageSharingMode() const noexcept;
  ...
  private:
    ...
    // The caller owns the backing allocation for at least as long as this AS.
    // Store its stable Vulkan identity rather than the movable Buffer object's address.
    vk::Buffer storageBufferHandle_{};
    vk::SharingMode storageSharingMode_ = vk::SharingMode::eExclusive;
...
//
// The current runtime contract records rebuilds only. Update and compaction
// flags are rejected until a production lifecycle owns those operations.
//
