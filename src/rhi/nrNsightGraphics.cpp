...
{
    return state_.config.activity != nr::platform::NsightGraphicsActivity::Off && state_.injected && state_.initialized;
}
...
void NsightGraphicsFrameHelper::beginFrame(bool vkFrameBoundaryEnabled)
{
    ...
    if (state_.config.activity == nr::platform::NsightGraphicsActivity::Capture && !state_.captureRequested &&
    ...
    if (state_.config.activity == nr::platform::NsightGraphicsActivity::Trace && state_.traceActivated &&
        !state_.traceRunning && state_.currentFrameOrdinal == *state_.targetFrame)
    {
        startTrace();
    }
}
...
{
    if (!enabled() || state_.config.activity != nr::platform::NsightGraphicsActivity::Trace || !state_.traceRunning ||
        state_.traceStopRequested || state_.currentFrameOrdinal < state_.traceStopFrameOrdinal)
    {
        return;
    }
...
[[nodiscard]] bool NsightGraphicsFrameHelper::textEqualsAny(std::string_view text,
...
{
    return text |
           std::views::transform([](char value) { return static_cast<wchar_t>(static_cast<unsigned char>(value)); }) |
           std::ranges::to<std::wstring>();
}

[[nodiscard]] std::optional<std::uint64_t> NsightGraphicsFrameHelper::parseUnsignedEnvironmentInteger(
...
[[nodiscard]] std::string_view NsightGraphicsFrameHelper::activityName(
    nr::platform::NsightGraphicsActivity activity) noexcept
{
    switch (activity)
    {
...
[[nodiscard]] nr::platform::NsightGraphicsCaptureDelimiter NsightGraphicsFrameHelper::captureDelimiter(
    bool vkFrameBoundaryEnabled) noexcept
{
    return vkFrameBoundaryEnabled ? nr::platform::NsightGraphicsCaptureDelimiter::VkFrameBoundaryExt
...
void NsightGraphicsFrameHelper::reportFailure(std::string_view operation,
                                              nr::platform::NsightGraphicsResult result) const
{
    if (result == nr::platform::NsightGraphicsResult::Success)
    {
        return;
