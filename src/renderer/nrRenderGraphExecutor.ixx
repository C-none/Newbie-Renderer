...
[[nodiscard]] inline std::optional<nr::rhi::DescriptorWritePayload> resolveLogicalDescriptorWriteDefault(
    const nr::rhi::LogicalResourceDescriptorWrite &logicalResource, const nr::rhi::DescriptorBindingInfo &binding,
    [[maybe_unused]] std::uint32_t arrayElement, const TContext &bindingContext)
{
    nrAssert(static_cast<bool>(bindingContext.resolveBuffer),
             "resolveLogicalDescriptorWriteDefault requires resolveBuffer callback.");
    nrAssert(static_cast<bool>(bindingContext.resolveImage),
             "resolveLogicalDescriptorWriteDefault requires resolveImage callback.");
    nrAssert(static_cast<bool>(bindingContext.resolveAccelerationStructure),
    ...
    auto resolveBufferDescriptor = [&]() -> std::optional<nr::rhi::DescriptorWritePayload> {
        ...
        return nr::rhi::DescriptorWritePayload{nr::rhi::BufferDescriptorWrite{
...
class RenderGraphExecutor
{
  public:
    ...
    [[nodiscard]] PreparedGraphFrame prepareFrame(const CompiledGraphFrame &compiled,
  ...
  private:
    ...
    static void attachFrameBoundaryMetadata(nr::rhi::CommandBatch &submitBatch, const ExecuteContext &context,
    ...
    [[nodiscard]] static vk::PipelineStageFlags2 imageAccessWaitStage(QueueDomain queue, const PassResourceUseDesc &use,
    ...
    [[nodiscard]] static vk::PipelineStageFlags2 imageAvailableWaitStageForBatch(
        ...
        const std::map<GraphResourceHandle, std::reference_wrapper<const CompiledResourceDesc>>
    ...
    [[nodiscard]] static bool canOmitQueueFamilyOwnershipTransfer(const CompiledResourceDesc &resource,
                                                                  const ResourceStateTransition &transition,
                                                                  const nr::rhi::Device &device);

    static void applyQueueFamilyTransferPolicy(CompiledGraphFrame &compiled, const nr::rhi::Device &device);
    ...
    static void resolveSwapchainRuntimeResources(const CompiledGraphFrame &compiled, const ExecuteContext &context,
                                                 std::uint32_t swapchainImageIndex, RuntimeBindingMap &runtimeBindings);

    [[nodiscard]] static std::size_t invokePassPrepareCallbacks(const CompiledGraphFrame &compiled,
    ...
    [[nodiscard]] static std::uint32_t preparedSecondaryPoolSlotCountForQueue(nr::rhi::FrameContext &frame,
                                                                              QueueDomain queue);

    [[nodiscard]] static std::uint32_t preparedRecordWorkerCountForQueue(nr::rhi::FrameContext &frame,
                                                                         QueueDomain queue);
    ...
    [[nodiscard]] vk::raii::CommandBuffer &primaryCommandBufferForQueue(const ExecuteContext &context,
                                                                        std::size_t frameSlot, QueueDomain queue,
                                                                        std::size_t ordinal);
    ...
    [[nodiscard]] static bool addTransitionBarrier(nr::rhi::ops::BarrierBatch &barriers,
                                                   const CompiledResourceDesc &resource,
                                                   const PreparedResourceBinding &binding,
                                                   const ResourceStateTransition &transition,
                                                   TransitionPlacement placement, std::uint32_t srcQueueFamilyIndex,
    ...
    [[nodiscard]] static std::uint32_t timestampValidBitsForQueue(const nr::rhi::Device &device, QueueDomain queue);
    ...
    [[nodiscard]] static std::uint64_t timestampDeltaTicks(std::uint64_t begin, std::uint64_t end,
    ...
    [[nodiscard]] static std::vector<GpuPassTimingSample> buildPassTimingSamples(const CompiledGraphFrame &compiled);

    static void ensureTimingQueryPool(const nr::rhi::Device &device, FrameGpuPassTimingState &state,
    ...
    [[nodiscard]] RecordBatchTasks launchRecordTasksForBatch(const ExecuteContext &context, std::size_t frameSlot,
    ...
    [[nodiscard]] static std::vector<RecordTaskResult> collectRecordTaskResults(RecordBatchTasks &tasks,
                                                                                std::size_t batchOrdinal,
                                                                                ExecuteReport &report);

    static void executeRecordedSecondaries(const vk::raii::CommandBuffer &primaryCommandBuffer,
                                           const CompiledSubmitBatch &batch,
                                           std::span<const RecordPassExecutionPlan> passPlans,
                                           std::span<const RecordTaskResult> results, vk::QueryPool timingQueryPool,
    ...
    static void recordImplicitCopyPass(const CompiledPass &pass, const vk::raii::CommandBuffer &commandBuffer,
