...
RenderGraphSkeletonPatchContext::RenderGraphSkeletonPatchContext(
    ...
    : frame_(frame)
    , layout_(layout)
    , namedFrameResources_(namedFrameResources)
    , namedFrameData_(namedFrameData)
    , globalResources_(globalResources)
{
}

void RenderGraphSkeletonPatchContext::patchResource(
...
void RenderGraphSkeletonPatchContext::patchFrameData(
    std::size_t localSlot,
...
void RenderGraphSkeletonPatchContext::patchPass(
    std::size_t localSlot,
    std::string_view debugName,
...
void RenderGraphSkeletonPatchContext::patchCopy(
    std::size_t localSlot,
    std::string_view debugName,
    CopyPassDesc copy)
{
...
[[nodiscard]] GraphResourceHandle RenderGraphSkeletonPatchContext::namedResource(
...
[[nodiscard]] GraphFrameDataHandle RenderGraphSkeletonPatchContext::namedFrameData(
    std::string_view name) const
{
...
[[nodiscard]] std::optional<std::reference_wrapper<const std::any>>
RenderGraphSkeletonPatchContext::resolveFrameDataPayload(GraphFrameDataHandle handle) const
{
...
[[nodiscard]] std::optional<RenderGraphSkeletonImageResourceDesc> RenderGraphSkeletonPatchContext::describeImageResource(
    GraphResourceHandle resource) const
{
    ...
    return std::visit(
        [](const auto& desc) -> std::optional<RenderGraphSkeletonImageResourceDesc> {
            using DescT = std::remove_cvref_t<decltype(desc)>;
            if constexpr (std::same_as<DescT, GraphTransientImageDesc> ||
                          std::same_as<DescT, GraphImportedImageDesc>)
            {
...
{
    auto found = std::ranges::find_if(entries_, [&](const Entry& entry) {
        return entry.key == key;
    });
    ...
    RenderGraphSkeletonCapture capture)
{
    ...
    if (skeleton.nodePatchLayouts.empty())
    {
        ...
        std::ranges::for_each(frame.nodes, [&](const GraphNodeDesc& node) {
            auto firstPass = std::ranges::find_if(frame.passes, [&](const PassExecutionDesc& pass) {
                return pass.node == node.handle;
            });
            auto passCount = static_cast<std::size_t>(std::ranges::count_if(
                frame.passes,
                [&](const PassExecutionDesc& pass) {
                    return pass.node == node.handle;
                }));
...
void RenderGraphSkeletonCache::refreshMaterialized(
    RenderGraphSkeletonKey key,
    const RenderGraphFrameDescription& frame,
    RenderGraphSkeletonCapture capture)
{
