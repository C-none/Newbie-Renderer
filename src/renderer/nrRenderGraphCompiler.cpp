...
[[nodiscard]] bool RenderGraphCompiler::hasExplicitSubmitBoundariesForQueueTransitions(
    const RenderGraphFrameDescription &frame)
{
    auto passQueueByHandle = std::map<GraphPassHandle, QueueDomain>{};
    std::ranges::for_each(frame.passes,
                          [&](const PassExecutionDesc &pass) { passQueueByHandle.emplace(pass.handle, pass.queue); });
...
[[nodiscard]] AccessScope RenderGraphCompiler::mapBufferAccessIntent(BufferAccessIntent intent,
                                                                     vk::PipelineStageFlags2 shaderStages)
{
    ...
    case BufferAccessIntent::TexelReadWrite:
        return AccessScope{effectiveShaderStages,
                           vk::AccessFlagBits2::eShaderStorageRead | vk::AccessFlagBits2::eShaderStorageWrite};
    case BufferAccessIntent::VertexRead:
        return AccessScope{vk::PipelineStageFlagBits2::eVertexAttributeInput,
    ...
    case BufferAccessIntent::AccelerationStructureRead:
        return AccessScope{vk::PipelineStageFlagBits2::eAccelerationStructureBuildKHR |
    ...
    case BufferAccessIntent::ShaderBindingTableRead:
        return AccessScope{vk::PipelineStageFlagBits2::eRayTracingShaderKHR,
...
[[nodiscard]] AccessScope RenderGraphCompiler::mapImageAccessIntent(ImageAccessIntent intent,
                                                                    vk::PipelineStageFlags2 shaderStages)
{
    auto const effectiveShaderStages = shaderStages != vk::PipelineStageFlags2{}
                                           ? shaderStages
    ...
    case ImageAccessIntent::ColorAttachmentRead:
        return AccessScope{vk::PipelineStageFlagBits2::eColorAttachmentOutput,
    ...
    case ImageAccessIntent::DepthStencilRead:
        return AccessScope{vk::PipelineStageFlagBits2::eEarlyFragmentTests |
...
[[nodiscard]] AccessScope RenderGraphCompiler::mapAccelerationStructureAccessIntent(
    AccelerationStructureAccessIntent intent)
{
    switch (intent)
    {
    ...
    case AccelerationStructureAccessIntent::CopyRead:
        return AccessScope{vk::PipelineStageFlagBits2::eAccelerationStructureCopyKHR,
                           vk::AccessFlagBits2::eAccelerationStructureReadKHR};
...
{
    auto const shaderStages = use.shaderStages != vk::PipelineStageFlags2{}   ? use.shaderStages
                              : passShaderStages != vk::PipelineStageFlags2{} ? passShaderStages
    ...
    const std::map<GraphResourceHandle, std::size_t> &resourceIndexByHandle)
{
    ...
    std::ranges::for_each(resourceUses, [&](const PassResourceUseDesc &use) {
        ...
        nrAssert(indexIt != resourceIndexByHandle.end(),
...
void RenderGraphCompiler::annotateResourceTransitions(CompiledGraphFrame &compiled)
{
    ...
    auto resolveLayout = [&](GraphResourceHandle handle, std::optional<ImageLayoutIntent> requestedLayout,
    ...
    std::ranges::for_each(compiled.submitBatches, [&](CompiledSubmitBatch &batch) {
        std::ranges::for_each(batch.passes, [&](CompiledPass &pass) {
            ...
            std::ranges::for_each(pass.resourceUses, [&](const PassResourceUseDesc &use) {
                ...
                auto resolvedLayout =
                    resolveLayout(use.resource, use.imageLayout,
                ...
                if (previousUse.has_value() && !previousFromSamePass)
                {
                    ...
                    else if (previous.layout.has_value() && resolvedLayout.has_value() &&
                    ...
                    if (strength != DependencyStrength::InOrder)
                    {
                        auto transition = ResourceStateTransition{
                            ...
                            .newLayout =
                                resolvedLayout.value_or(previous.layout.value_or(ImageLayoutIntent::Undefined)),
                            .strength = strength,
            ...
            std::ranges::for_each(passFinalUse,
                                  [&](const auto &pair) { lastUse.insert_or_assign(pair.first, pair.second); });
        });
    });
...
[[nodiscard]] std::string RenderGraphCompiler::makeDebugView(const CompiledGraphFrame &compiled)
{
    ...
    std::ranges::for_each(compiled.resources, [&](const CompiledResourceDesc &resource) {
        ...
        text += std::format(
            "resource[{}] {} type={} lifetime={} residency={} usage(buffer={}, image={}) layout={}=>{} "
            "ownership={}=>{}\n",
            resource.handle.value, resource.debugName, typeName, static_cast<std::uint32_t>(resource.lifetime),
            static_cast<std::uint32_t>(resource.residency), static_cast<std::uint32_t>(resource.resolvedBufferUsage),
            static_cast<std::uint32_t>(resource.resolvedImageUsage), static_cast<std::uint32_t>(resource.initialLayout),
            static_cast<std::uint32_t>(resource.finalLayout), static_cast<std::uint32_t>(resource.initialOwnership),
    ...
    std::ranges::for_each(compiled.submitBatches, [&](const CompiledSubmitBatch &batch) {
        ...
        text += std::format("submitBatch[{}] queue={} passCount={} openedBySubmit={}\n", batch.batchIndex,
                            queueName(batch.queue), batch.passes.size(), openedBy);
        ...
        std::ranges::for_each(batch.passes, [&](const CompiledPass &pass) {
            text += std::format("  pass[{}] {} isCopyPass={} queue={} uses={} resolvedUses={} preBarriers={}\n",
                                pass.handle.value, pass.debugName, pass.isCopyPass ? 1 : 0, queueName(pass.queue),
                                pass.resourceUses.size(), pass.resolvedResourceIndices.size(), pass.preBarriers.size());
        });
    });
    ...
    std::ranges::for_each(compiled.ownershipTransitions, [&](const ResourceStateTransition &transition) {
        text += std::format("ownershipTransition resource={} {}->{} layout={}=>{}\n", transition.resource.value,
                            queueName(transition.srcQueue), queueName(transition.dstQueue),
