...
Scene material textures use this convention through a single global combined image sampler table: `gSceneTextures[]` is declared at Vulkan set 1, binding 0. Shader-visible material texture IDs default to 0, where the renderer binds a 1x1 purple fallback texture; resident scene textures use `TextureHandle.slot + 1` as their descriptor index. Material texture slot enums only define push-constant/draw-packet ordering and do not create additional set 1 bindings. Raster material texture IDs are stored as 16-bit values and packed two IDs per 32-bit push-constant lane; `kSceneTextureDescriptorCapacity` must fit that ABI. `shader/materialTextureIds.slang` owns the shader-visible slot constants and packed-ID unpack helpers so raster, GBuffer, and RT shaders share one unpack ABI. C++ render passes bind the table through the shared `nr.renderPasses:sceneTextureTableBinding` helper, which handles frame-slot allocation, fallback prefill, version checks, resident-slot updates, and removed-slot fallback rewrites.
