...
- Consumers reach legacy third-party declarations and generated dependency-boundary declarations through narrow named C++ modules such as `dependency.vulkan`, `dependency.vma`, `dependency.window`, `dependency.math`, `dependency.ui`, `dependency.assets`, `dependency.slang`, `dependency.ecs`, `dependency.nsight`, and `dependency.shaderShare`; [`src/extern/exportDependency.ixx`](../../src/extern/exportDependency.ixx) remains a compatibility umbrella that re-exports them.
...
- The bundled Slang submodule is configured and installed into the build tree as a local package before the `dependency` target links the imported `slang::slang` target. Before `dependency` module compilation, [`src/extern/tools/nrShaderShareCodegen.cpp`](../../src/extern/tools/nrShaderShareCodegen.cpp) reflects data-only declarations under [`shader/include/share`](../../shader/include/share) through `shader/common.slang` and emits the generated `dependency.shaderShare` module. For shared `enum : uint` types the same pass also emits `SlangEnumMeta`/`slangEnumLiteral` enumerator-name reflection so C++ can spell Slang enum literals from the translated enum (a codegen stopgap pending C++26 static reflection); see [`shader/SHADER_NAMING_AND_ORGANIZATION.md`](../../shader/SHADER_NAMING_AND_ORGANIZATION.md).
...
`Renderer::installGraph(spec)` installs long-lived nodes and an optional generic frame-resolution resolver once -> each `renderFrame(input)` obtains one of the existing three free `FrameSlot`s -> resolver graphs acquire first, while ordinary graphs defer acquire -> renderer resolves display/render extents from post-acquire presentation state when required -> optionally drives display-sized `scene` extraction and bridge building (with optional app-side camera override) -> forwards optional per-frame service state through `FrameServices` -> collects optional node UI sections in installed pipeline order -> builds and compiles the graph using the resolved plan -> prepares/submits batches -> at the compiled acquire boundary either acquires late or validates/reuses the pre-acquired image -> records/submits the swapchain copy batch -> presents on the main thread
...
- `RendererGraphSpec::frameResolutionResolver` is API-agnostic: empty resolvers produce an identity `FrameResolutionPlan` and preserve late acquire, while a non-empty resolver requires the graph's single compiled acquire boundary, pre-acquires with `RendererFrameInput::acquireTimeout`, then receives `nr.rhi::Device` plus the post-acquire display extent and returns validated display/render extents and a temporal reset. `NodeFrameParameters::swapchainExtent` remains the display extent. The executor validates the pre-acquired image/index and current swapchain extent/format before binding it at the boundary; image-available waiting, final submission, and present semantics are unchanged. Scene extraction and default camera projection use display extent; pixel-space camera jitter converts to NDC with render extent and propagates the plan reset through `RendererCameraFrameState`.
...
- Renderer owns one app-global environment image independently of scene/model and installed-graph lifetime. `Renderer::setEnvironmentMap(...)` synchronously uploads the GPU-only RGBA16F image through `UploadReadbackContext`, preserves retained shader-read layout/access/graphics ownership across frames, and exposes its RDG handle plus 16-byte decode/intensity/yaw parameters through `FrameGlobalResources`. Initialization installs a 1x1 legacy-miss-color fallback; the viewer replaces it once with the default EXR before model loading, and model reloads or pipeline switches do not re-upload it.
- renderer frame-resource keys include the RT-only material and geometry-atlas sideband published by the AS node: `sceneRtInstanceMetadata`, `sceneRtGeometryMetadata`, `sceneRtMaterialHeaders`, `sceneRtMaterialLayers`, `sceneRtMaterialTextureRefs`, `sceneRtVertexAtlas`, and `sceneRtIndexAtlas`. The AS node also publishes keyed graph frame data `scene.rt.hitSbtPlan`, which is the CPU-side RT hit SBT plan consumed by PathTracing during graph build. `LightPrepareNode` publishes `sceneLightHeader`, `sceneLights`, and `sceneLightAliasTable` as the frame-local scene light upload. Shader-visible RT nodes bind those buffers through `RayTracingPassBuilder::uniform(...)` / `storageBuffer(...)` in the same prepare/record split used by other descriptor-backed resources.
...
- Renderer owns the node UI collection contract: `NodeRuntime::collectUi(...)` can add custom command/state `NodeUiSection`s through `NodeUiBuildContext`, and `NodeFrameParameters::nodeUiSections` exposes the current frame's sections to the UI render node. The app layer supplies the concrete `NodeUiWriter`; UI callbacks mutate node-owned staged state so changes become visible on the next frame. Nodes own display labels, display values, range validation, runtime-only UI state, and any mapping from UI state to link-time shader variants.
- `NodeConfig.instanceName` is the required, single source for an installed node's runtime name. Renderer uses it for initialization, graph nodes, node UI section IDs, build contexts, submit defaults, and node-owned pipeline debug labels; Graphics remains the default queue domain.
...
- PathTracing binds the renderer-global environment through its dedicated reflected `gEnvironmentMap` combined sampler (linear, repeat longitude, clamp latitude) and `gEnvironment` push constants. Only material-ray escape/miss radiance and the primary alpha-blend background sample it; visibility rays skip the sample, and the environment is deliberately absent from scene-light alias tables, direct-light sampling, and NEE. The missing-TLAS/sideband branch still executes the existing seven-image clear path before environment binding, so a frame with no model preserves current clear behavior.
- `AccumulateNode` is the selectable non-RR post-processing mode for `rtobject`; the graph installs exactly one Accumulate or DLSS RR node in its post-processing slot. Accumulate owns its unjittered view/projection snapshot and history sample count, and discards its ping-pong history when that camera transform changes or its history images are recreated; it does not consume renderer or DLSS reset state.
- `DlssRayReconstructionNode` remains disabled by default as a reusable external compute node and preserves every DLSS 310.7.0 RR image slot, SDK-defined subrect base, create flag, temporal scalar, matrix, and research-guide toggle for programmatic configuration. Its renderer-collected UI session exposes Enable, Quality, one preset for each of the five quality modes, DLAA-only output bypass, Preset E/Depth of Field guidance, motion-vector visualization with its conditional legend, one-shot history reset, runtime status, and queried optimal settings. All other create/evaluate settings, create flags, optional-resource inclusion, and subrect controls are programmatic-only through `DlssRayReconstructionNodeInput`; non-DLAA bypass is cleared by UI staging and rejected for programmatic input. The default `rtobject` RR graph owns a bounded five-quality controller and installs the generic renderer resolver. The resolver reads the node's pending-or-active staged request before UI commit, caches exact NGX optimal settings per quality for one display extent, clears that cache on display resize, returns identity when disabled, and marks enable/quality/display/render transitions for temporal reset. PathTracing produces at the resolved render extent. Coordinated RR forbids local size overrides, validates the controller snapshot against active node input and the renderer plan, targets display extent, and reuses the early optimal settings without a duplicate query; standalone RR retains its programmatic overrides and late query path. Bypass still evaluates NGX and may publish the input color only in DLAA, so disabled RR cannot send a low-resolution PathTracing image directly to Present. Automatic NGX matrices use the renderer-selected unjittered `NodeFrameParameters::renderCameraConstants`, including app/viewer overrides, while subpixel jitter is supplied separately and normalized by render extent. Evaluation reset combines the resolution-plan flag, node-local one-shot, and feature-lifecycle reset state. The node calls the typed `nr.rhi:dlss` RAII feature from its RDG record callback. When requested, [../../shader/renderer/dlssRayReconstructionDebug.slang](../../shader/renderer/dlssRayReconstructionDebug.slang) subsequently visualizes the same scaled pixel-space MV input into RR's color output without skipping NGX or resetting history; outside this explicit graph wiring, its default output remains `dlss.rr.output.*`.
...
The app-global environment follows a parallel, scene-independent path: default `assets/envMap/studio_small_09_8k.exr` -> `nr.load:exr` -> `nr.resource:environment` scaled RGBA16F -> renderer upload ring and retained image -> PathTracing material-ray miss sampling. It is not registered in `nr.scene` and is not an environment light/NEE input.
...
- Viewer startup loads [../../src/pipeline/nrPipelineEnvironment.cpp](../../src/pipeline/nrPipelineEnvironment.cpp)'s default studio environment once after renderer initialization and before model loading; a missing or invalid configured environment is a startup error, while renderer-only consumers retain the 1x1 fallback.
