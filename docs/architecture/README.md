...
`Renderer::installGraph(spec)` installs long-lived nodes once -> each `renderFrame(input)` obtains one of the existing three free `FrameSlot`s -> optionally drives `scene` extraction and bridge building (with optional app-side camera override) -> forwards optional per-frame service state through `FrameServices` -> collects optional node UI sections in installed pipeline order -> builds the graph -> compiles through the renderer/RDG cache suite -> prepares and submits pre-acquire batches -> acquires one swapchain image at the graph control boundary -> records/submits the swapchain copy batch -> presents on the main thread
...
- `DlssRayReconstructionNode` remains disabled by default as a reusable external compute node and preserves every DLSS 310.7.0 RR image slot, SDK-defined subrect base, create flag, temporal scalar, matrix, and research-guide toggle for programmatic configuration. Its renderer-collected UI session exposes staged core settings, flags, MV visualization, and status; optional-resource inclusion and subrect controls are intentionally omitted from the UI. DLSS RR is the default `rtobject` post-processing selection; that graph enables its instance in full-resolution DLAA mode, uses packed normal/roughness plus hardware depth, activates only the six required resources and specular hit distance, and routes its output to `frameResource::presentSourceColor`. Automatic NGX matrices use the renderer-selected unjittered `NodeFrameParameters::renderCameraConstants`, including app/viewer overrides, while subpixel jitter is supplied separately. Evaluation reset combines the renderer-owned camera reset flag with node-local one-shot and feature-lifecycle reset state; the renderer flag is currently always false. The node calls the typed `nr.rhi:dlss` RAII feature from its RDG record callback. When requested, [../../shader/renderer/dlssRayReconstructionDebug.slang](../../shader/renderer/dlssRayReconstructionDebug.slang) subsequently visualizes the same scaled pixel-space MV input into RR's color output without skipping NGX or resetting history; outside this explicit graph wiring, its default output remains `dlss.rr.output.*`.
