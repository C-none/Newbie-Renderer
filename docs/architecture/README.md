...
- `Renderer::installGraph(...)` asks every `NodeRuntime` for its ordered, CPU-only `shaderRequests()`, flattens all static requests into one `ShaderService::compileProgramsByFile(...)` batch, validates the complete result, and only then initializes nodes with their corresponding `NodeInitContext::shaderPrograms` slice. Static PSO construction therefore cannot start until the shared six-worker backend batch has completed. Scene-derived permutations such as PathTracing hit-group variants remain deferred to the owning node, where one pipeline miss still collects and compiles the complete required batch before RT PSO construction.
...
- `PathTracingNode` consumes `frameResource::sceneTlas`, the RT metadata/material buffers, scene vertex/index atlas resources, published scene light buffers, and `frameData::sceneRtHitSbtPlan`, then publishes `frameResource::presentSourceColor` as a fixed 1spp single-frame result. It derives `PathTracingVariantKey` once per frame from the immutable option snapshot and retains no UI draft or pending setting. Its node-local runtime cache remains split by responsibility: `PathTracingPipelineKey` combines the active path-tracing policy with the hit-group permutation-set hash and owns RT PSO creation, while `PathTracingSbtKey` adds the hit-record-plan hash and owns SBT creation. On a pipeline miss, the node collects every required single-entry request and submits one `ShaderService::compileProgramsByFile(...)` batch before creating the RT PSO; SBT misses rebuild only the shader-binding table. Raygen, miss, any-hit, and closest-hit live in [raygen.slang](../../shader/renderer/pathTracing/raygen.slang), [miss.slang](../../shader/renderer/pathTracing/miss.slang), [anyHit.slang](../../shader/renderer/pathTracing/anyHit.slang), and [closestHit.slang](../../shader/renderer/pathTracing/closestHit.slang). Only raygen receives `kMaxSurfaceBounces` and `RussianRoulettePolicy`; miss and any-hit are non-variant; each closest-hit request receives `kEnableFilterAfterShading` plus its `CHS = MaterialCHS<RtMaterialLayerFlag(N)>` assignment. FAS therefore changes closest-hit codegen without becoming a second CHS generic or material/SBT key dimension. The AS/SBT plan retains 17 BSDF layer-flag combinations and a binary any-hit policy for at most 34 hit groups. Alpha-mask geometry and single-sided geometry exposed by an instance-wide cull disable select the shared any-hit policy; ordinary opaque geometry keeps the hardware fast path. Actual Slang entry-point names are discovered from each single-entry program for Vulkan, while explicit logical names such as `ch_<hash>` drive shader-group lookup. Hit records sharing a permutation reuse the same pipeline group handle, and groups sharing a BSDF key reuse the same closest-hit stage. Material, visibility, and shadow-style rays use hit SBT offset/stride `0/1`. Focused helper modules still own scheduling, path state, hit reconstruction, material resolution, direct lighting, environment evaluation, roulette, and output. `ahMaterialPolicy` applies single-sided back-face rejection and alpha-mask coverage with `IgnoreHit()`; if the frame lacks a complete TLAS/RT sideband plan, the node clears its outputs instead of running RT consumers.
...
- PathTracing transports resolved hit data through a dedicated packed `ResolvedMaterialRayPayload` rather than the larger BSDF working records. Its reflected layout is exactly 32 32-bit lanes (128 bytes): flags, indices, world position, base color, emissive, transmission IOR, scatter weights, and eta remain full-width; five unit directions use standard oct32 encoding; and ten bounded material scalars use five UNORM16 pairs. Closest hit is the sole result encoder, while raygen immediately decodes into the full-precision material/scatter working records, re-normalizes directions, reprojects the anisotropy tangent, and restores the minimum roughness floor. Closest-hit input, hit output, and miss output continue to reuse physical slots whose lifetimes do not overlap.
