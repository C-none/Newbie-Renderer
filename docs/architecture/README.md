...
- Renderer additionally owns the opt-in, Release-only RenderGraph Skeleton benchmark export boundary: `RendererBenchmarkConfig` starts a warmup/measure/drain session, captures rendered-frame CPU records in memory, joins delayed per-pass GPU timestamps by the renderer's monotonic frame ordinal rather than recycled frame slot, and writes mode-aware v3 artifacts only during finalization. The pipeline/viewer CLI selects the benchmark-only Legacy or Enabled mode before RT graph installation, while the artifact audit rejects mode-inconsistent telemetry; detailed schema and interpretation live in [../renderer_performance_measurement.md](../renderer_performance_measurement.md).
