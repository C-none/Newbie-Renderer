...
- [../../src/pipeline/exportModule.ixx](../../src/pipeline/exportModule.ixx) and [../../src/pipeline/nrPipeline.cpp](../../src/pipeline/nrPipeline.cpp) provide the single viewer runtime used by the executable. The registered `normalview` graph is `NormalBuffer -> Ui -> Present`; the registered `rtobject` graph uses graphics `AccelerationStructureBuild -> LightPrepare -> PathTracing -> Ui` -> submit -> pre-acquire compute `DlssRayReconstruction -> Present.Convert` -> acquire -> compute `Present.CopyToSwapchain`.
