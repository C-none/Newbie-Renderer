...
- `PathTracingNode` consumes `frameResource::sceneTlas`, the RT metadata/material buffers, scene vertex/index atlas resources, published scene light buffers, and `frameData::sceneRtHitSbtPlan`, then publishes `frameResource::presentSourceColor` as a fixed 1spp single-frame result. It derives `PathTracingVariantKey` once per frame from the immutable option snapshot, converts the snapshot's bounce and roulette values into Slang variant assignments, and retains no UI draft or pending setting. Its node-local runtime cache is split by responsibility: `PathTracingPipelineKey` combines the root path-tracing variant with the active hit-group permutation set hash and owns RT PSO creation, while `PathTracingSbtKey` adds the hit record plan hash and owns SBT creation. These PSOs are not stored in `RendererCacheSuite`. Pipeline misses synchronously compile only Slang variants required by the current scene and create the RT PSO on the graph build thread; SBT misses rebuild only the shader-binding table for the active record plan. The AS/SBT plan keys the BSDF by `RtMaterialLayerFlag` plus the independent `RtBaseLobeVariant` (17 valid combinations: one isotropic unlit program plus eight lit layer masks in isotropic and anisotropic forms), then pairs that key with a binary any-hit policy for a hard upper bound of 34 hit groups. Alpha-mask geometry and single-sided geometry exposed by an instance-wide cull disable select the shared material policy; ordinary opaque geometry keeps the hardware fast path. Alpha blend, double-sided, volume-boundary, and emissive flags remain runtime material behavior. Anisotropy texture presence remains runtime material data rather than a specialization dimension. The node compiles the fixed [../../shader/renderer/pathTracing.slang](../../shader/renderer/pathTracing.slang) root with the global path-tracing variant, then compiles one lightweight CHS link variant per unique BSDF key as `MaterialCHS<RtMaterialLayerFlag(N), RtBaseLobeVariant(M)>` (Slang link-time specialization splits unlit vs lit, base distribution, and inactive layers); RHI stage selection uses actual entry-point names such as `chMain` for Vulkan stages and logical names such as `ch_<hash>` for shader-group lookup. Hit SBT records are not deduplicated, but records with the same hit-group permutation reuse the same pipeline shader group handle, and groups with the same BSDF key can reuse the same closest-hit stage. Material, visibility, and shadow-style rays use hit SBT offset/stride `0/1`, so Vulkan selects `instanceShaderBindingTableRecordOffset + GeometryIndex()`. The shader entry keeps the ray tracing entry-point ABI (`rgMain`, `msMain`, `ahMaterialPolicy`, `chMain`) and constructs a scheduler from focused `shader/renderer/pathTracing/*` modules: `Scheduler` owns exactly one raygen path per pixel, `Pt` owns the per-path `PathState` plus trace/hit/miss/output methods, closest-hit reconstructs the hit triangle from `InstanceID()`, `GeometryIndex()`, `PrimitiveIndex()`, RT metadata, and scene atlas data through [../../shader/include/rt/resources.slang](../../shader/include/rt/resources.slang) and [../../shader/include/rt/hitSurface.slang](../../shader/include/rt/hitSurface.slang), and closest-hit delegates material resolution plus BSDF scatter sampling to the link-time `CHS : ICHS` contract in [../../shader/include/pathTracing/chs.slang](../../shader/include/pathTracing/chs.slang). Raygen owns pixel- and monotonic-sample-frame-based RNG seeding, direct lighting from glTF photometric scene lights, finite-distance visibility rays for punctual lights, miss/emissive contribution, roulette, and next-bounce path-state updates. Point/spot direct lighting uses inverse-square attenuation with the optional glTF range fade. `ahMaterialPolicy` applies single-sided back-face rejection and alpha-mask coverage with `IgnoreHit()`; alpha blend plus thin and volume transmission remain opaque-like unless geometry policy requires any-hit filtering. If no TLAS or complete RT sideband is published for the frame, it clears its output instead of requiring RT consumers to run.
- RT material texture sampling selects `TEXCOORD_0` or `TEXCOORD_1` per dense slot, applies that slot's affine transform once, and uses the original explicit mip-0 `SampleLevel` policy for base-color/alpha, normal, metallic-roughness, emissive, clearcoat, sheen, transmission, and anisotropy reads. This material anisotropy feature does not introduce derivative-footprint sampling, hardware anisotropic filtering, or Filter After Shading. ID 0 remains the generic neutral-white fallback; authored-but-unavailable anisotropy is the explicit semantic exception and decodes `(1, 0.5, 1)` while RT/TLAS wait behavior remains strict for all other slots. Raster readiness remains strict for anisotropy as well. The occlusion slot and its transform remain in the shared transport ABI for fidelity, but current shading deliberately does not sample or multiply AO.
- Lit CHS variants expose one `BaseSurfaceBsdfLobe<LayerFlags, BaseLobeVariant>` entry for diffuse reflection, dielectric/metal reflection, and transmission. The base-only masks `1/3/5/7` and transmission masks `9/11/13/15` remain distinct linked programs; transmission record reads and BTDF code sit behind the existing static layer-bit branch and are removed from the base-only family at link time. The transmission-enabled family selects thin folded-GGX or volume rough-dielectric behavior at runtime from the shared layer record. IOR zero preserves the `KHR_materials_ior` positive-infinity compatibility sentinel and forces the final interface Fresnel to one. Volume transmission uses the raw triangle front/back face, Snell refraction, TIR, and a bounded four-entry strict-LIFO IOR boundary stack keyed by the collision-free dense `InstanceID()` metadata index plus `GeometryIndex()` (with material index as a consistency component), with RR-only eta compensation; nested disconnected shells packed into one BLAS geometry are outside this boundary identity and must be split into separate geometries. It deliberately stores no traveled distance or absorption/scattering state. Thin transmission does not modify that stack.
- `KHR_materials_anisotropy` is carried by the RT header as strength, counter-clockwise rotation, and explicit usable-texture presence. The dense anisotropy texture ref retains UV selection and affine transform; the shader samples it with explicit `SampleLevel`, decodes RG direction and B strength, and builds its frame from the final normal-mapped base normal. The anisotropic variant changes only the base lobe's GGX distribution, separable Smith masking, visible-normal PDF, and VNDF sampler. Reflection, thin folded transmission, and volume transmission share those helpers; clearcoat remains isotropic, and no derivative-footprint or hardware anisotropic texture filtering participates in this material feature.
