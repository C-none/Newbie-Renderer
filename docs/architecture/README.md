...
- Pass record callbacks are worker-capable and record into executor-retained secondary command buffers. The executor queues one-shot `std::packaged_task` work items across all compiled submit batches, then gathers each batch's `std::future` results only when the main thread reaches that batch and executes the recorded secondaries on the primary command buffer in compiled pass order.
